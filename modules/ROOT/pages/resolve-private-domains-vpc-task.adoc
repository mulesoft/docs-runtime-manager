...
* The maximum number of characters of all the domains you configure must not exceed 239 characters. For example, adding the domains `mydomain1.com` and `mydomain2.com` requires 28 characters. Each domain name consumes an additional character because in the DNS hierarchy all domains end with a "." character.
* Avoid entering hostnames to ensure that you do not exceed the 239 character limit.
