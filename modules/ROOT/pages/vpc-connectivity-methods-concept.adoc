...
To create a Direct Connect connection to the CloudHub VPC, create an AWS Transit Gateway on your AWS account (or leverage an existing one you have on AWS) and set up the Direct Connect connection to the Transit Gateway. Then attach the CloudHub VPC to the Transit gateway and configure the required routing to enable CloudHub VPC to On-premise connectivity.
