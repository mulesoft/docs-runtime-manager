...
When using static routing, the remote networks will be added to the route table when the tunnel is active, and removed when the tunnel is inactive. This is the expected bahviour.
When using BGP routing, the routes are propagted from your VPN endpoint when the tunnel is active. If the route table does not show the required entries, please ensure the BGP session is active, and validate the BGP configuration on your device.
