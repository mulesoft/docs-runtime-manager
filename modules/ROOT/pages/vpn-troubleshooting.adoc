...
The VPN connection supports only one Security Association (SA) pair per tunnel, so any more than one traffic selector per connection results in traffic problems.
