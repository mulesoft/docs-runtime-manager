...
Because of the inherent delay in the infrastructure-provider API that is queried to retrieve the tunnel status, delays up to five minutes in reporting the correct status are expected. We strongly recommend against using this information to trigger a failover between the tunnels (tunnels should be monitored at the customer-side VPN gateway).
