...
For example, you can define application-specific properties in a properties file, then later add environment-specific properties from Runtime Manager during deployment.
...
The values of properties defined on this tab override any application properties defined in the `mule-app.properties` or other properties of Mule.
...
* By default, Runtime Manager does not display the values of the properties.
* Only users that have access to the *Settings* page can view properties.
...
. Click *Save* and then click *Redeploy Application*.
