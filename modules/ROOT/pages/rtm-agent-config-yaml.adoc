...
Some agent components allow you to configure them during runtime. For information, see xref:administration-service.adoc[Administration Service].
...
When you register Mule runtime engine with Runtime Manager, the generated `mule-agent.yml` disables the REST agent transport and replaces any existing configuration.
...
* Cloudwatch publishers allow users to send JMX metrics to Amazon Cloudwatch.
...
This option is only supported in version 1.5.2 or later of Runtime Manager agent.
