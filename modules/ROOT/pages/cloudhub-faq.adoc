...
=== Is there a limit on the total number of applications I can have?

Yes, in addition to limiting vCores, Cloudhub limits the number of applications per environment per organization.
This limit is calculated by multiplying the allocation of vCores in the environment by 20.
For example, if your Sandbox environment has 2 vCores allocated, you can have a total of 40 applications with 0.1 vCores  (20 running applications and 20 not-running applications, each with 0.1 vCores) in the same environment at the same time. 
