...
You can assign a static IP to your application by using the *Static IPs* application settings tab. For details, see xref:deploying-to-cloudhub.adoc#static-ips[Static IPs]. By default, you are allocated a number of static IPs equal to twice the number of production vCores in your subscription. To raise this limit, contact MuleSoft Support.
