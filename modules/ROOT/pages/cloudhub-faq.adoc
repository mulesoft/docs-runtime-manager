...
* If a xref:virtual-private-cloud.adoc[Anypoint Virtual Private Cloud (Anypoint VPC)] is built for your organization, any existing application that is not already in the Anypoint VPC receives a new IP address when it restarts inside the Anypoint VPC.
* An application is redeployed to a different geographic region. You can pre-allocate a static IP in the new region in the *Static IPs* tab in the application settings page.
