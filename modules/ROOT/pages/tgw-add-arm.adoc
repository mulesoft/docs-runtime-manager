...
. <<add_tgw_routes,Add routes to the AWS VPC>> in the transit gateway route table. 
. <<add_tgw_aws_routes,Add routes from the AWS VPC>> on AWS.
...
After the *VPC attached* message appears, add routes for the AWS VPC to the transit gateway route table on Anypoint Platform so that apps deployed to the VPC 
can access the transit gateway and the transit gateway routes traffic correctly.
