...
. <<add_tgw_routes,Add routes from the Anypoint VPC to the AWS VPC>>. 
. <<add_tgw_aws_routes,Add routes from the AWS VPC to the Anypoint VPC>>.
...
After the *VPC attached* message appears, add routes for the AWS VPC to the transit gateway route table for the VPC so that apps can access the transit gateway and the transit gateway routes traffic correctly.
...
[[add_tgw_aws_routes]]
== Add Routes to the Transit Gateway on AWS

After successfully adding routes to the transit gateway route table in Anypoint Platform, 
add the Anypoint VPC to the route table for the AWS VPC.

. In AWS, select *Services* > *VPC*. 
. In the *VPC Dashboard*, click *Route Tables*.
. Click the route table ID that corresponds to the VPC.
. In the *Routes* section, click *Edit routes*.
. Under *Destination*, click *Add route* and enter the route (the CIDR block of your Anypoint VPC).
. Under *Target*, select *Transit Gateway*, and then click *Save changes*.
