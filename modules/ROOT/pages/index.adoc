...
Anypoint Runtime Manager is the interface to Anypoint Platform that provides a unified view of your applications, servers and APIs, no matter where they are running. Runtime Manage enables you to monitor and analyze your Mule applications from one central location, whether your apps are deployed in the cloud, on-premises, or on Runtime Fabric.

Runtime Manager is designed specifically for handling Mule applications, and deploying them to the available Mule runtime instances in a xref:cloudhub.adoc[CloudHub] virtual worker, or an on-premises Mule server (server group, cluster), or Runtime Fabric. 
