...
After you create a Mule application (for example, see xref:general:getting-started:build-a-hello-world-application.adoc[Hello World Mule application]), you can deploy it anywhere through Runtime Manager. Runtime Manager is designed specifically for handling Mule applications, and deploying them to the available Mule runtime instances in a xref:cloudhub.adoc[CloudHub] virtual worker, or an on-premises Mule server (server group, cluster, and so on), or a Runtime Fabric. 
