...
[IMPORTANT]
The Loadbalancers do not support HTTP Streaming at this point. When a call is made directly to the worker HTTP streaming behaviour is demonstrated as expected (i.e An error response is received before the full attachment is uploaded)
However when a call is made to the Loadbalancer the response is not received until the full upload is complete (this could take many minutes).
