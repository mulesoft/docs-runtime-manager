= Extending Runtime Manager Agent for Hybrid
...
This topic contains an overview of how to extend Anypoint Runtime Manager agent. It provides instructions on implementing a complete service, external handler and internal hander using a method to revert a string.

== Runtime Manager Agent Extensible Component

Runtime Manager agent contains the following extensible components:
...
**
 * <p>
 * Implementations of this interface provides new functionality to Runtime Manager agent. These services handle data from the
