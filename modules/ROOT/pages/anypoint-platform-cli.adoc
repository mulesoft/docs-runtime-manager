...
This document assumes that you are familiar with our xref:1.x@api-manager::tutorials.adoc[API Manager Tutorials] and that you are able to xref:2.x@api-manager::tutorial-manage-an-api.adoc[manage your API using Anypoint Platform].

== Installation

[IMPORTANT]
--
Anypoint-CLI is a https://nodejs.org/en/[Node.js] application. Before you can install Anypoint CLI on your computer, you first need to make sure you have the Node.js runtime environment installed on your computer. Along with it, you'll need to use the default Node.js package manager (npm) to be able to install Anypoint CLI.
...
The CLI also allows you to pass the token for your account using the `--bearer` option. +
You need to hit the +https://anypoint.mulesoft.com/accounts/login+ endpoint with your username and password to get your bearer value, and then pass that value to the CLI:
...
Environment variables override credentials file parameters and Command line parameters override environment variables. If you don't pass a command line parameter, the default profile properties are used.
