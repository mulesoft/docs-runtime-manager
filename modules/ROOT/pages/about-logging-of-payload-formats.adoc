...
Because some payload formats can't be converted to plain text, Runtime Manager agent only converts those formats that are considered to work. 
...
The following table lists the payload formats and whether Runtime Manager agent converts them to String and passes the payload message.
...
You can override the default behavior and configure the agent to forward all payload formats or none by adding
properties to the `conf/mule-agent.yml` file.
...
[IMPORTANT]
If you force conversion for all payload formats, the payload might break.
