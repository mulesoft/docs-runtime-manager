...
Because some payload formats can't be converted to plain text, Runtime Manager agent only converts those that are considered to work. 
When the payload format can't be converted, the agent passes only the fully qualified name of the payload class type, not its content.
