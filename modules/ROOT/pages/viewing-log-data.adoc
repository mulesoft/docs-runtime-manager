...
CloudHub provides access to log data that includes deployment messages and events for each worker and logs for Mule runtime engine.
...
The runtime log contains information about app and lifecycle events.
For example, the log records an entry when a Mule service or app starts, deploys, stops, or undeploys.

. In the *Deployment* filters section on the right, find the date you want to view.
. Click the download icon next to the worker, and select *Mule Logs*:
+
.The arrow shows the icon to download Mule log output by deployment on the *Logs* page.
image::cloudhub-logs-download.png[Download Mule log output on the Logs page]
