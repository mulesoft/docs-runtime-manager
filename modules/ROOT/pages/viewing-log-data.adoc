...
[NOTE]
====
Logs and diagnostics are currently available only for applications deployed to CloudHub workers. In cases where applications are deployed to a local server, you can send data to external analytics software. 
...
. In Runtime Manager, click *Applications* in the left menu, then click the application that is deployed to CloudHub to open the management panel on the right. 
. Click *Logs*. The application logs are displayed.
...
The log file is a text file with `.log` file extension and named in the format of `<deployment-ID>-<instance-ID>.log`. You can view the log file with a text editor.

[NOTE]
If you need to archive or download CloudHub application logs for audit, analytics, or other purposes at regular intervals, use the xref:custom-log-appender.adoc[Custom Log Appender] to extract logs.
...
You can also access the following URL to download a diagnostics file directly:
