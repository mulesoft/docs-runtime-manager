...
To create a DLB and enable TLS 1.0 support using the CLI: 

. Run the following command to recreate the SSL endpoint for the _myLB_name_ load balancer and add TLS 1.0 support: 
+
[source,linenums]
----
cloudhub load-balancer create --tlsv1 myVPC myLB_name certificateName privateKey
----
. Run the following command to set the _certificateName_ as the default SSL endpoint for the _myLB_name_ load balancer: 
+
[source,linenums]
----
...
To enable TLS 1.0 support on an existing DLB using the CLI: 
