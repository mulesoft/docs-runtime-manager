...
You can disable CloudHub logs and integrate your CloudHub application with your logging system using the Log4j configuration.
...
For logs to flow to both your logging system and be viewable in CloudHub, configure the CloudHub Log4j appender. The Log4j appender sends log data to CloudHub and to a local log file.
...
For information on configuring `log4j2.xml`, see the
...
. In the *Disabling CloudHub logs* confirmation windows, select the checkboxes to verify that you want to disable CloudHub Loggers the server group:
+
