= Troubleshooting
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:keywords: cloudhub, managing, monitoring, troubleshooting, runtime manager, arm, applications, servers

image:logo-cloud-active.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-active.png[link="/runtime-manager/deployment-strategies", title="Hybrid"]
image:logo-server-active.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]
image:logo-rtf-disabled.png[link="/runtime-fabric/deploy-to-runtime-fabric", title="Runtime Fabric"]



If you experience problems with your deployed apps, Runtime Manager provides several tools that can help you determine the root cause.

Depending on your deployment strategy, there is a different set of possible troubleshooting resources available. 

See xref:monitoring.adoc[Monitoring] for details on the monitoring tools of the Runtime Manager, which can also be of help you diagnose an issue.

== Troubleshooting Applications

You can access information about applications using the following tools:

* xref:insight.adoc[Insight]
image:logo-cloud-active.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-active.png[link="/runtime-manager/deployment-strategies", title="Hybrid"]
image:logo-server-disabled.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]
image:logo-rtf-disabled.png[link="/runtime-fabric/deploy-to-runtime-fabric", title="Runtime Fabric"]
+
Access a dashboard with key metrics, see events at an individual transaction level of detail.


* xref:api-functional-monitoring::index.adoc[API Functional Monitoring]
image:logo-cloud-active.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-active.png[link="/runtime-manager/deployment-strategies", title="Hybrid"]
image:logo-server-active.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]
image:logo-rtf-active.png[link="/runtime-fabric/deploy-to-runtime-fabric", title="Runtime Fabric"]
+
Using API functional monitoring, you can automate testing and generate reports to verify API functionality and validate the results that the API generates. You can monitor APIs at each stage of the software development life cycle and in production.


* xref:viewing-log-data.adoc[View application log data]
image:logo-cloud-active.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-disabled.png[link="/runtime-manager/deployment-strategies", title="Hybrid"]
image:logo-server-disabled.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]
image:logo-rtf-disabled.png[link="/runtime-fabric/deploy-to-runtime-fabric", title="Runtime Fabric"]
+
Access a log of events occurring to your application and the servers or CloudHub workers they run on.


For applications deployed to your own servers, you can export log data to third party software to view there. 

== Troubleshooting Servers

image:logo-cloud-disabled.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-active.png[link="/runtime-manager/deployment-strategies", title="Hybrid"]
image:logo-server-active.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]
image:logo-rtf-disabled.png[link="/runtime-fabric/deploy-to-runtime-fabric", title="Runtime Fabric"]

With applications deployed to your own servers, you can access information through the following tools:

* xref:monitoring.adoc#monitoring-servers[Monitor servers]
+
Use alerts and dashboards to get information about the servers.
* xref:sending-data-from-arm-to-external-analytics-software.adoc[Export data to external analytics tools] 
+
Send data to an external tool like Splunk or ELK.



== See Also

* xref:managing-deployed-applications.adoc[Managing Deployed Applications]
* xref:deploying-to-cloudhub.adoc[Deploy to CloudHub]
* xref:alerts-on-runtime-manager.adoc[Alerts]
