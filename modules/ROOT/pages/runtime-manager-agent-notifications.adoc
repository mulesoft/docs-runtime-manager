...
== Configuring the Mule Agent Notifications Callback URL

To start receiving the notifications in the desired URL, you must first configure the `callback` URL in the `mule-agent.yml` file inside the REST transport section as shown below.

+
[TIP]
Make sure to enable the `rest.agent.transport` section or the notification will not be sent. Also ensure the configured url ends with a trailing slash (`/`). This is required since the agent will append the path to it.
