...
[NOTE]
--
This is an optional addition to your cloud infrastructure that is not included by default in all CloudHub plans. To enable it, you must first own a xref:virtual-private-cloud.adoc[Virtual Private Cloud offering].
...
Although this feature will eventually be built into the Runtime Manager UI, currently it’s only available as a service that can be used via the xref:anypoint-platform-cli.adoc[Anypoint Command Line Interface] or using the xref:runtime-manager-api.adoc[Cloudhub API] through the endpoints `anypoint.mulesoft.com/cloudhub/api/organizations/{orgid}/vpcs/{vpcId}/loadbalancers` and `anypoint.mulesoft.com/cloudhub/api/organizations/{orgid}/vpcs`.

[NOTE]
====
For a full list of Anypoint-CLI commands and their respective usage, check our xref:anypoint-platform-cli.adoc#List-of-commands[reference page]. +
...
You can set an order when creating the rule using the xref:anypoint-platform-cli.adoc#cloudhub-add-lb-rule[cloudhub-add-lb-rule] command in the Anypoint-CLI by specifying an index value.
