...
** Public key file (Combined Primary.crt & Intermediate_CA.crt into one file.  File should be converted to PEM extension.): `example-com-crt-public.pem`
** Private key file (Should be an unencrypted private key -> openssl rsa -in example-com-private.pem -out example-com-private_noencrypt.pem): `example-com-private-noencrypt.pem`
