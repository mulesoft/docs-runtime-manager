...
To send insights and monitoring information, Runtime Manager agent calls the `auth-proxy` service. To connect to this service, the agent reads the proxy configuration from the `$MULE_HOME/conf/mule-agent.yml` file.

The agent uses WebSockets to communicate with both Anypoint Platform consoles. You can configure this as a WebSockets transport in the `$MULE_HOME/conf/mule-agent.yml` file or in the `$MULE_HOME/conf/wrapper.conf` file.
