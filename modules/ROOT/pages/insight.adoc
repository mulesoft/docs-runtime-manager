...
[IMPORTANT]
====
Enabling Insight may cause a performance impact when processing application data. We strongly discoraged enabling Insight in production environments for long periods of time. To monitor a production environment, we recommend using the tools listed in the xref:monitoring.adoc[monitoring section].
