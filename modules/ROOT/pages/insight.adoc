...
Use the Insight Dashboard to view events that occur within the flows that handle your business transactions.
Drill down into event-related data to analyze the root cause of failures, isolate performance bottlenecks, and test for compliance with company procedures.
...
In these cases, any Streams that are not mapped to String by default appear in the logs. 
