...
Insight does not support visibility into batch processing. See xref:mule-runtime::batch-processing-concept.adoc[Batch Processing].
...
[NOTE]
*Metadata and Replay* tracking option is no longer supported for free and trial users.
...
Replay is available for Mule 3 apps only. It is not supported for Mule 4 apps.
