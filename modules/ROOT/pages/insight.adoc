...
NOTE: To enable the *Business Data* field, your app must have <<set_up_business_events,business events>> configured.


The *Contains* option returns a match if the search string is found at the start of the item. For example, if the search criteria is "Employee Email contains jan", then "jano" and "janet" match, but not "alejandro".
