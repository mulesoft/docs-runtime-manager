...
Use Insight only for troubleshooting specific transactions by searching for an exact term, 
such as transaction ID or a unique text string.
...
* Insight does not return consistent results when searching for terms that are not an exact match.
+
Because Insight limits the number of terms that are included in the search, it might not search for all available terms. To work around this limitation, search for an exact term.
