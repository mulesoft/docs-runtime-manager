...
For Mule 3 apps, Insight supports the ability to replay transactions for advanced error recovery. 
...
=== Replay Transaction (Mule 3 apps)
...
. Click the replay icon (circular arrow) to the right of the first event in the failed transaction to replay it:
+
.The screenshot shows (*1*) the replay icon and (*2*) the *Purge Replay Data* button.
...
|`STALLED`|The event has been put on hold due to the maximum number of events to be processed per transaction (10000). Any event above this mark is set as stalled and then processed after this event is no longer stalled.
...
+
When deploying your application with Runtime Manager, select either *Metadata* or *Metadata and Replay* on the *Insight* tab of the *Settings* page.
