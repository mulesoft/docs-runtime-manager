...
[NOTE]
====
Anypoint Platform Private Cloud Edition doesn't support Insight.
====
...
Using the Insight Dashboard, you can view events that occur within the flows that handle your business transactions. You can drill down into event-related data to access information that can help you analyze the root cause of failures, isolate performance bottlenecks, and test for compliance with company procedures.

[IMPORTANT]
====
Enabling Insight may cause a performance impact when processing application data. Enabling Insight for long periods of time is not recommended for production environments. To monitor a production environment, use the tools listed in the xref:monitoring.adoc[monitoring section].
...
[NOTE]
If you want to enable insight on apps that run on servers on-premises, you must obtain special entitlements for your account. Contact your MuleSoft sales representatives to obtain these.
...
* Metadata: Tracks customized key/value pairs that you specify as part of custom events in your application. The information that Insight tracks at this level presents information about high-level business events.
...
For transaction-level events, Insight may display long numeric values in the ID column. Rather than use random number identifiers, you can customize the transaction IDs so that they display more intuitive values that are relevant to your business requirements.
...
New applications and applications whose region changed after Oct 7, 2017 will start using region-specific storage. Insights Replay Data is not preserved when switching regions. Verify that this doesn't violate any of your compliance requirements.
...
For example, a transaction fails because an application exceeds the limited number of API calls for a Salesforce account. Insight displays the status of the transaction as `Failed`. 
