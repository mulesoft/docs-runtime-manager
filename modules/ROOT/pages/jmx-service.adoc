...
To add a new JMX publisher, you need to implement the Runtime Manager agent `InternalMessageHandler` interface.
...
MuleSoft provides several modules for Runtime Manager agent, including JMX publishers for Nagios, Zappix and others. These modules are an open source project, and are not supported by MuleSoft.

To access the Runtime Manager agent modules, check the GitHub repositories:

* https://github.com/mulesoft/mule-agent-modules[agent Modules]
