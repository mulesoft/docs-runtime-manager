...
Switching between the two groups of static IP addresses enables zero-downtime restarts and updates and ensures no transactions are dropped or lost because the client respects the DLB's DNS TTL. (*what does this mean?*)

All newly created DLBs with static IP use this static IP model. 
For existing DLBs that use static IP addresses, the Runtime Manager *Load Balancers* page displays *Update Required* until you update the DLB:
