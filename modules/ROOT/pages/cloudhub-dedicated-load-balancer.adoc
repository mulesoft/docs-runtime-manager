...
Since, static IPs will change when restarting or updating your DLB, it is recommended to use DLB's DNS address rather than the IP addresses when calling it from an external service. Ensure that the client includes the necessary retry mechanisms to account for DNS switch.

[NOTE]
For DLBs created before August 2021, click the *Update* button to avail the feature. After clicking *Update*, the DLB uses static IP model described above and you cannot revert to the old model.
