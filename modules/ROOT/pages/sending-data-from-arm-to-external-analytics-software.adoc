...
* Mule v3.6 or later, or API gateway standalone version 2.1.0 or later if you deal with apps that are built with the API gateway.
...
[NOTE]
--
Starting from Mule Runtime versions 4.2.0 and 3.9.3 the runtime can be configured to forward events to both a third-party system, like ELK and Splunk, and to Anypoint Platform by adding an additional property to the `wrapper.conf` file:
