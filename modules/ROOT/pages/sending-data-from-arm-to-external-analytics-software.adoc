...
[NOTE]
Sending data to third party software is currently not (out of the box) available for applications on xref:cloudhub.adoc[CloudHub].
One way this can be achieved is via the https://docs.mulesoft.com/runtime-manager/custom-log-appender[CloudHub Custom Log Appender.]
