...
[NOTE]
====
Although the Runtime Manager agent exists by default bundled with Mule as of version 3.7, all Mule versions older than 3.7.2 contain an old version of the Runtime Manager agent that doesn't support this feature. You must download the latest version of the Runtime Manager agent separately if your Mule version is older than 3.7.2.

http://mule-agent.s3.amazonaws.com/1.2.0/mule-agent-1.2.0.zip[Download the Runtime Manager Agent]
====
...
Note that three extra arguments must be added to what you copied from the Runtime Manager UI: 
...
[TIP]
For more information on how to install or update the Runtime Manager agent, see xref:installing-and-configuring-runtime-manager-agent.adoc[Install or Update the Runtime Manager Agent].
