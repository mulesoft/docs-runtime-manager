...
To integrate with Splunk, you must create a new source type on your Splunk instance with the correct configuration to parse the HTTP events sent by the Runtime Manager agent.
...
You can configure the Runtime Manager agent to send event notifications from Mule runtime flows to a configurable log file.
Logstash captures and indexes the data.
...
== Integrating API Analytics with Splunk or ELK
