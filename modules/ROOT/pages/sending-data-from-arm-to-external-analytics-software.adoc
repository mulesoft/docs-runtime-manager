= Exporting Data to External Analytics Tools
...
You can configure Runtime Manager to export data to external analytics tools.

Using either the Runtime Manager cloud console or Anypoint Platform Private Cloud Edition, you can:

* Send Mule event notifications, including flow executions and exceptions raised to Splunk or ELK.
+
See <<Integrating with Splunk>> and <<Integrating with Your ELK Stack>>.
* Send API Analytics to Splunk or ELK.
+
See <<Integrating API Analytics with Splunk and ELK>>.


If you plan to export your payload's contents to be logged, keep in mind that not all formats can be exported.
...
You can use the Log4j appender to integrate with your logging system. 
See xref:custom-log-appender.adoc[CloudHub custom log appender].
...
. <<Configure Splunk in Runtime Manager>>. 
...
+
[IMPORTANT]
If you set values for the Splunk index, Splunk source, and Splunk source type when you registered your data input in Splunk, any values you specify in this window overwrites those values.
...
|Splunk Index
a|Splunk index name to which to send all events

If the index doesn't exist, the internal handler creates it if the user has permission.
|main
...
|Splunk Type 
|The sourcetype used for events sent to Splunk
|mule
...
== Integrating with Your ELK Stack

ELK (Elasticsearch, Logstash, Kibana) consists of three open-source tools that help you store, search, and analyze log data.

You can output Mule event notifications as generic system logs, which can be handled by your ELK stack.
Logstash captures and indexes the data into the log, from which you can then use Elasticsearch and Kibana to generate graphs, reports, alerts, dashboards, and visualizations.
The Runtime Manager agent helps you store event notifications produced from the Mule runtime flows into a configurable log file with a rolling file policy.

=== Configure the Log File for Your ELK Stack

To send event notifications to the folder that your ELK stack reads from:
...
+
The `.<n>` in the code snippets refers to an integer that is unique within `wrapper.conf`.
...
After you've configured your API gateway, configure Splunk or ELK in Runtime Manager:
...
. Click the gear icon and continue with step 8 to complete the configuration:
+
* <<Configure Splunk in Runtime Manager>>
* <<Configure the Log File for Your ELK Stack>> 
