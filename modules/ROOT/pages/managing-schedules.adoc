...
* If a scheduled job is not triggered because the application is down,
...
+
.The screenshot shows (*1*) the *Logs* option, (*2*) the *Last Run* column, and (*3*) the frequency
link on the *Schedules* page.
image::cloudhub-scheduler-rtm.png[Logs option and the Last Run column on the Schedules page]
+
