...
The Anypoint Runtime Manager agent is a Mule runtime engine plug-in that exposes the Mule API. Runtime Manager (both the cloud console managed by MuleSoft and Anypoint Platform Private Cloud Edition) uses this agent to communicate with its registered Mule instances.
