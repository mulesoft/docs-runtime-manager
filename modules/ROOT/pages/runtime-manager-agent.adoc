...
The Runtime Manager Agent is a plugin extension for a Mule runtime engine that exposes the Mule API. Runtime Manager (both the cloud console and the Anypoint Platform Private Cloud Edition) uses this agent to communicate with its registered Mule runtimes on servers.

Outside the Runtime Manager, you can use the Runtime Manager Agent to monitor and control your Mule servers by calling Runtime Manager Agent APIs from external systems, and/or have Mule publish its own data to external systems. The Runtime Manager Agent has many features, including:
...
[NOTE]
The Runtime Manager is available only for Enterprise versions of Mule runtime engine, not for Community versions. Currently, the Runtime Manager Agent supports Mule 3.6.0 or later.


[NOTE]
If you're deploying your applications to xref:deploying-to-pcf.adoc[Pivotal Cloud Foundry], then you don't need to worry about installing or configuring the Runtime Manager Agent. In this scenario, the Runtime Manager Agent functions as usual as the link between the Mule runtime engines and the Runtime Manager. However, the same PCF buildpack that automatically creates new runtime instances also creates Runtime Manager Agent instances and registers these to the Runtime Manager. If you want to change the configuration of your Runtime Manager Agent instances, you must modify the buildpack.
...
Once configured, the Runtime Manager Agent securely connects to this Runtime Manager account every time the Mule runtime engine restarts.  The link that the Runtime Manager Agent establishes allows the Mule runtime engine to be monitored and managed remotely through an Anypoint Runtime Management Console.

If this link goes down while the Mule runtime engine is still running, statistics and analyics are buffered until the WebSocket connection can be re-established. Additionally, there is also a reconnection strategy in place in cases the WebSocket connection drops.
...
In addition to configurating and allowing REST connections with a Runtime Manager Agent, you can configure a Runtime Manager Agent plugin to connect a Mule runtime engine to a variety of external systems. The installation script provides options to choose particular communication methods and external systems, including controlling a Mule runtime engine through a Runtime Manager.
...
The pages listed below assume that you have the current version of Mule runtime engine already installed. Download it from the http://www.mulesoft.com/support-login[customer portal] if you need to upgrade to this version.
