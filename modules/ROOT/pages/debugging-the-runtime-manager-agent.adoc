...
You can use the `$MULE_HOME/bin/amc_debug` script to collect information used by Support to troubleshoot Runtime Manager agent installation issues. If you experience issues with the Runtime Manager agent plugin or with managing applications on Anypoint Platform, run the script and save the output before filing a support ticket.
