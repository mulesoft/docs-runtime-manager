...
MuleSoft Anypoint VPN is a managed service that is underpinned by AWS Site-to-Site VPN connections. VPN is key for any integrations or flows that require connectivity to your on-premises or other external platforms. Updating an Anypoint VPN that is not set up with at least a pair of tunnels using BGP routing will cause downtime. See xref:vpn-high-availability.adoc[Anypoint VPN High Availability] for more information.

After the October 2022 quarterly maintenance window, Anypoint VPN will no longer be able to support quarterly customer self-upgrades. From this point forward, updates made to Anypoint VPN will adhere to AWS's standard continuous upgrade process. Upgrades will be applied on-demand, with no published maintenance window scheduled, no customer opt-in self-upgrade capabilities, with *potential (limited) downtime for customers that do not have a* xref:vpn-high-availability.adoc[High Availability (HA) VPN implementation].

MuleSoft recommends that all customers plan to deploy a xref:vpn-high-availability.adoc[high-availability VPN] solution to avoid any downtime associated with these AWS delivered security updates. Please review the below section for recommendations and troubleshooting.

== Avoiding or Reducing Downtime During an on-demand maintenance

Mule applications that use Anypoint VPN connectivity are impacted during the on-demand maintenance. However, the impact on the Anypoint VPN varies depending on the VPN configuration:
...
These are some of the possible issues you may encounter during the on-demand maintenance:
...
If there is no traffic over the VPN tunnel, it goes down due to inactivity. Generate interesting traffic. The recommendation is for customers to move towards BGP.
