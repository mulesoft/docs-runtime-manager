...
To ensure the security and stability of CloudHub, MuleSoft releases security updates
and monthly patches and applies them automatically on a schedule.

Automatic updates use blue-green deployment, which ensures zero downtime.
...
To minimize disruption, the monthly date patch update always includes patches for any P1 security issues.
P0 security patches might be included in the monthly update or released separately, depending on the SLA requirements.
...
The monthly date patch update might also include security patches.
...
* First Monday of the month, MuleSoft releases the patch.
+
You can apply the update manually or let MuleSoft apply the patch automatically.
+
See <<Update the Runtime Version Manually>>
* Third Saturday of the month, MuleSoft automatically applies the patch.
+
