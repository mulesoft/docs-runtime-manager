...
xref:virtual-private-cloud.adoc#vpc-connectivity-methods[VPC connectivity methods] (VPC Peering and CloudHub Direct Connect) are not a self-serve function at this time. +
...
The SLA for configuring VPC is five business days after the VPC discovery form is completed and returned, though it may be completed sooner.

== Update an Existing VPC

These instructions are for updating VPCs created before November 2018. If you are updating an IPsec VPN tunnel that was created after November 2018, you can https://support.mulesoft.com/s/article/How-to-add-an-additional-Static-route-to-an-Anypoint-VPN-tunnel [add additional static routes] using the API.
