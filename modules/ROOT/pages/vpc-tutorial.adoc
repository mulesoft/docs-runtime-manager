...
Make sure you thoroughly understand how to properly size the Anypoint VPC for your needs before configuring this parameter. If you are not sure of how to configure this setting, follow the  xref:virtual-private-cloud.adoc#size-your-vpc[Anypoint VPC sizing] guide.
--
+
...
xref:virtual-private-cloud.adoc#vpc-connectivity-methods[Anypoint VPC connectivity methods] (Anypoint VPC Peering and CloudHub Direct Connect) are not a self-serve functions at this time. +
To configure a connection between your Anypoint VPC and your infrastructure, download link:{attachmentsdir}/VPC-form-v9.3-template.xlsx[the Anypoint VPC discovery form] (requires at least MS Excel 2007 with macros enabled), enter the necessary information required for your connectivity method and send it to MuleSoft customer support.

The SLA for configuring Anypoint VPC is five business days after the Anypoint VPC discovery form is completed and returned, though the Anypoint VPC configuration may be completed sooner.

== Update an Existing Anypoint VPC

These instructions are for updating Anypoint VPCs created before November 2018. If you are adding additional static routes to an IPsec VPN tunnel created after November 2018, you https://support.mulesoft.com/s/article/How-to-add-an-additional-Static-route-to-an-Anypoint-VPN-tunnel [can use the API] to do so.
