...
== Apply Required VPN Update Manually  

The *Update* button indicates a VPN requires an update.
The VPN update includes performance and security enhancements.

You can update the VPN manually at your convenience or 
wait for CloudHub to apply the update manually.

Depending on your configuration, the update might cause downtime for up to 15 minutes.

To update a VPN manually:

. From Anypoint Platform, click *Runtime Manager*.
. Click *VPNs* and select the VPN to update.
. Click the *Update* button.
. At the *Update VPN* prompt, click *Update Now*.
+
Click the *Learn more* link for details about the update. 


== Update Anypoint VPN Connection Settings
