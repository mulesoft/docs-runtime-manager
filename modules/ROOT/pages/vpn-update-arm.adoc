...
You can't reserve or reuse external IP addresses. When you recreate the VPN, new local external IP addresses are assigned.
...
The local network is inherited from Anypoint VPC. You can't subnet the Anypoint VPC CIDR block.
...
. Update the Border Gateway Patrol (BGP) configuration on your VPN endpoint
+
See the documentation for your device.
+
The route table automatically updates when the changes are propagated by the BGP.
. Review the firewall rules for the corresponding Anypoint VPC.
+
See xref:vpc-firewall-rules-concept#add-or-edit-firewall-rules[Add or Edit Firewall Rules].
+
