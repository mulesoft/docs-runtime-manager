...
* xref:deploying-to-cloudhub.adoc#deploy-an-application-to-cloudhub-from-runtime-manager[Runtime Manager]
* xref:deploying-to-cloudhub.adoc#deploy-an-application-to-cloudhub-from-studio[Anypoint Studio]
...
* xref:deploying-to-cloudhub.adoc#deploy-from-cloudhub-cli[CloudHub CLI]
* xref:mule-runtime::deploy-to-cloudhub.adoc[Mule Maven plugin]


== Deploy an Application from Runtime Manager to CloudHub
...
To deploy to a local server, see xref:deploying-to-your-own-servers.adoc[Deploying to Your Servers].
--
...
*Logging* tab::
Configure the logging level for the application: DEBUG`, `WARN`, `ERROR`, or `INFO`.
+
...
== Deploy an Application from Studio to CloudHub

You can deploy your applications directly from Studio to CloudHub.
This is helpful if you're still developing the application and want to deploy it often to an online test environment.

[NOTE]
--
You cannot deploy an application from Studio to the CloudHub *Design* environment.
The *Design* environment is specific to applications created in Anypoint Platform Design Center.
--
...
Application names must be globally unique across CloudHub to avoid domain conflicts. 
...
== Set Up a Deployment Failed Alert

To be notified by email when there is an error during deployment, set up a deployment failed alert:

image::deployfailedalert.jpg[DeployAppFirstScreen]

Complete the form and choose *Deployment failed* as the alert condition.
...
Select this box to enable persistent queues on your application. Persistent queues protect against message loss and enable you to distribute workloads across a set of workers. Before you can take advantage of persistent queueing, your application must be set up to use queues. See xref:cloudhub-fabric.adoc[Clustering] for more information.
...
== Changing a Deployed Application
