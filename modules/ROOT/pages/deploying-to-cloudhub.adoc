...
Applications are always deployed with public application names in the `cloudhub.io` domain. Application names must be globally unique across CloudHub to avoid domain conflicts. 

For this reason, create a naming convention for your applications. For example, you can always prefix your applications with `mycompany` and optionally a department name: `mycompany-mydept-myapplication`. You can then add your own DNS records to hide this complex application name. For example, you can route requests to `myapplication.mycompany.com` to `mycompany-mydept-myapplication.cloudhub.io`.
...
[Note]
While deploying an application on mule runtime versions 4.1.x and above, the https://docs.mulesoft.com/api-manager/2.x/gatekeeper#how-gatekeeper-works[gatekeeper] mechanism is in place to help determine the readiness of a deployment. If gatekeeper considers the application not ready in 5 mins, then the deployment will be considered a failed one.
This mechanism is used in conjunction with the https://docs.mulesoft.com/api-manager/2.x/api-auto-discovery-new-concept[API Autodiscovery].
