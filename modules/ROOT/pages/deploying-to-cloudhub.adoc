...
When you deploy your application, enter a unique application name. Valid names have a minimum of 3 and a maximum of 42 characters and can contain alphanumeric characters and dashes. Runtime Manager alerts you if the name is available or already reserved by another user.
...
[NOTE]
An application with the same name cannot be deployed in two environments at the same time.
...
+
You can also click *Get from sandbox* to copy a file from a non-production environment into your current environment. (If the *Get from sandbox* button does not appear, you first need to create a non-production environment.)
...
If you need to increase this quota, you contact https://www.mulesoft.com/support-and-services/mule-esb-support-license-subscription[MuleSoft Support].
