...
* *Runtime Manager* - In the cloud-based version of the https://anypoint.mulesoft.com[Anypoint Platform]. Go to the *Runtime Manager* section, then from the *Applications* tab click *Deploy application*.
...
To move an app from a sandbox environment to production:
...
In the *Deploy Application* dialog, enter a unique application name. Valid names have a maximum of 42 characters, which can contain alphanumeric characters and dashes. If the name is valid, Runtime Manager alerts you as to whether it is available or already reserved by another user.
...
After deploying your application, you can allocate the amount and size of the worker associated with your application. On each application, workers are responsible for executing the application logic.
