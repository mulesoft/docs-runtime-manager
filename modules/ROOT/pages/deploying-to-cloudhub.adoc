...
The application is always deployed with a public application name in the `cloudhub.io` domain. The application name must be globally unique across CloudHub to avoid domain conflicts. 

For this reason, create a naming convention for your applications. For example, you can always prefix your applications with `mycompany` and optionally a department name, for example `mycompany-mydept-myapplication`. You can then add your own DNS records to hide this complex application name, so for example you can route requests to `myapplication.mycompany.com` to `mycompany-mydept-myapplication.cloudhub.io`.
