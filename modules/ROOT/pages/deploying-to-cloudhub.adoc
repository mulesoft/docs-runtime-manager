...
image::cloudhub-deploy-app.png[Configuration tabs on the Deploy Application page]
+
...
[NOTE]
--
You can deploy an application from Studio to any environment in except *Design*.
...
The application name identifies your application in Runtime Manager and also used for the 
public application name in the `cloudhub.io` domain.
For example, an application named `myapplication` is accessible at `+http://myapplication.cloudhub.io+`.

The application name must be globally unique across CloudHub to avoid domain conflicts.
Runtime Manager prevents you from choosing a name that is already in use.

To avoid domain conflicts, create a naming convention for your applications, such as adding a prefix to 
your applications: `mycompany-mydept-myapplication`.
You can then add DNS records to hide this complex application name.
For example, you can route requests to `myapplication.mycompany.com` to `mycompany-mydept-myapplication.cloudhub.io`.

With the CloudHub dedicated load balancer, you can optionally implement mapping rules to set your application's public URL to any other path, as long as you own the domain.
...
+
Once the app is deployed, CloudHub automatically alerts you when updates for the selected runtime version are available. 
...
Workers::
When deploying an application with more than one worker, CloudHub automatically balances any incoming traffic load across your allocated workers.
...
+
--
* For applications deployed to Europe, the domain is automatically updated to +http://myapplicationdomain.eu.cloudhub.io+`
* For applications deployed to Asia-Pacific, the domain is automatically updated to `+http://myapplicationdomain.au.cloudhub.io+`
--
...
+
See xref:object-store::object-store.adoc[Object Store V2].
...
You can pre-configure IP address rules for multiple regions for disaster recovery (DR) scenarios.
...
If you need to free up some of your static IP allocations, you can release unused IP addresses from an application.
If you need to increase the quota, contact MuleSoft Support.
...
* Provide limited CPU and IO for smaller workloads.
* Have the ability to burst to higher CPU speeds for a short duration of time.
+
This ability helps to improve application startup times and process infrequent large workloads.
...
To avoid domain name conflict, an application with the same name cannot be deployed in two environments
at the same time.
You must modify the application name to deploy it to another environment.

Runtime Manager prevents you from duplicating applications in a single environment, so you cannot move an application into the environment it's already in.
To deploy the same or similar application in a single environment, modify the filename of one application.

To move an application from sandbox to production:

. Sign in to Anypoint Platform.
. Select *Runtime Manager* and change to your production environment.
. In the *Applications* page, click *Deploy application*.
. Provide a name for your application and click *Get from sandbox*.
. Select your sandbox environment and select the application to move to production.
...
. Click *Apply* and continue setting up your deployment.


== Deploy a Proxy from API Manager

You can deploy an API proxy directly from API Manager to CloudHub without using Runtime Manager.
