...
Use transit gateway attachments to create a secure connection between your MuleSoft Virtual Private Cloud (VPC) and your private corporate networks. You can add multiple transit gateway attachments if required. 
...
You must specify the routes (subnets) in your network that you want to be accessible through the transit gateway attachments.
...
Transit gateway attachments don't support these features and configurations:
...
* https://docs.aws.amazon.com/vpc/latest/tgw/tgw-vpc-attachments.html[Transit gateway attachments to a VPC]