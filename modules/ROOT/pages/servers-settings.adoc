= Access Server Settings (Hybrid)
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]

image:logo-cloud-disabled.png[link="/runtime-manager/deployment-strategies", title="CloudHub"]
image:logo-hybrid-active.png[link="/runtime-manager/deployment-strategies", title="Hybrid Deployment"]
image:logo-server-active.png[link="/runtime-manager/deployment-strategies", title="Anypoint Platform Private Cloud Edition"]

Use the *Settings* page to configure server properties and view alerts. You can also perform other actions from this page, such as delete a server group.  

To access the server *Settings* page:

. From Anypoint Platform, select *Runtime Manager*.
. Click *Servers* in the left menu.
. Either: 
+
** Click the *Type* column to display the details pane, then click *Manage Server*, *Manage Group*, or *Manage Cluster* button:
+
.The arrow shows the *Manage Server* button in the details pane.
image::rtm-view-server-settings.png[*Manage Server* button]
** Click the name of the server, server group, or cluster on the *Servers* page, then click *Settings*.
+
.The arrow shows option to navigate to the *Settings* page.
image::rtm-server-settings-page.png[*Settings* page]



== See Also

* xref:alerts-on-runtime-manager.adoc[Alerts on Runtime Manager]
* xref:servers-properties-view.adoc[Configure Server Properties]
* xref:servers-properties-create.adoc[Create Custom Server Properties]
* xref:servers-actions.adoc[Shut Down, Restart, or Delete Servers]
