...
After you deploy an application through the Runtime Manager, you can use the console to monitor and manage it while running. Depending on the xref:deployment-strategies.adoc[deployment strategy] used (where the application is deployed to and from what console), there is a different set of possible actions you can perform on it.
...
On applications deployed to xref:managing-applications-on-your-own-servers.adoc[your own servers] you can additionally do the following:
...
On applications deployed to xref:managing-applications-on-cloudhub.adoc[CloudHub] you can additionally manage the following elements:
...
For more on the options offered by this tab, see xref:managing-servers.adoc[Managing Servers].
...
You can also click on the *Insight* button to see data at a transaction level. See xref:insight.adoc[Insight] for more info about what you can see here.

image::managing-deployed-applications-76124.png[]

[NOTE]
The Dashboard and insights are features that are currently not available for the Anypoint Platform Private Cloud Edition. See xref:deployment-strategies.adoc[deployment strategies].

== Monitoring Applications

Depending on the xref:deployment-strategies.adoc[deployment strategy] used (where the applications are deployed to, and through what console), different tools exist for monitoring events that may occur on your application and the performance of the servers or virutal servers it runs on. See xref:monitoring.adoc[Monitoring Applications] for more details.
...
You can set up email alerts that are sent whenever certain events occur to your application, such as deployment failures or application deletions. These alerts may be linked to a specific application or to all of them. See  xref:alerts-on-runtime-manager.adoc[Alerts] for instructions on how to do this.
