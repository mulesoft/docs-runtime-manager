...
|image::managing-deployed-applications-a665b.png[] |*Stopped*|The application is not running.
...
|image:console-overview-red-circle.png[console_overview_red_circle] |*Deployment Failed* | Application deployment failed.

The cause of the failure appears in the console.

| image::managing-deployed-applications-ab70d.png[] ||Application update failed.
| image::managing-deployed-applications-de1b7.png[] |*Undeployed*|Application's status is unknown, as the server is not responding to the platform (on-premises deployments only)
...
If you're deploying your application to an on-premises cluster, Runtime Manager installs the new app version simultaneously on each server to ensure no downtime.
