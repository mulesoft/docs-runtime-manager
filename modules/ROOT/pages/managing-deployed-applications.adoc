...
Replica statuses apply only to applications deployed on Runtime Fabric. These statuses appear when you are applying new, or changed, configurations to deployed applications. See 1.0@runtime-fabric::app-versioning.adoc[Application Deployment Versioning] for more information.
...
What this dashboard displays depends on if your application is deployed on xref:managing-applications-on-cloudhub.adoc[CloudHub] or on xref:managing-applications-on-your-own-servers.adoc[a Mule server]. You will always see the status, the time of the last modification, and the deployed application ZIP file. You can click the *Choose File* button to change the application ZIP file.
...
To update applications deployed to Runtime Fabric, see 1.0@runtime-fabric::app-versioning.adoc[Application Deployment Versioning]
...
[NOTE]
Switching an alert off from this view only switches it off for the current (signed in) user. Other users can still have active alerts.
...
* 1.0@runtime-fabric::app-versioning.adoc[Application Deployment Versioning]
