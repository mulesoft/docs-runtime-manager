...
To use ECDSA based authentication cipher, you must generate the certificate and key pair using ECDSA based algorithm and apply it to the SSL certificate and key configuration on the DLB.

If you try to use ECDSA based authentication cipher with RSA based certificate and key pair, the SSL handshake will fail and you will receive a either of the following error messages:
...
Current DLB configuration only allows for one type of key pair for an SSL endpoint. You can configure the DLB with SSL certificate and key that are generated either with RSA or ECDSA algorithm but not both.

[NOTE]
If you configure a DLB with an ECDSA based key pair, clients using RSA based ciphers won't be able to connect to that DLB.
