...
The CloudHub load balancer can optionally verify client requests against certificate revocation lists (CRL). All CRL files must be concatenated into a single, unencrypted PEM-encoded file for upload. The order of the CRL is not important.
...
If your load balancer already exists, you can use the https://anypoint.mulesoft.com/apiplatform/anypoint-platform/#/portals/organizations/68ef9520-24e9-4cf2-b2f5-620025690913/apis/8617/versions/85955[REST API] to update it. To send a `PATCH` request to the `/organizations/{orgid}/vpcs/{vpcId}/loadbalancers/{lbId}` endpoint, add a `revocationList` element:
...
Use the CloudHub REST API to programmatically update your revocation lists.
...
The following recommended ciphers suites provide a good balance between compatibility and security for your SSL endpoint. All cipher suites offer forward secrecy except RC4-SHA, which supports Internet Explorer 8.
