...
Current DLB configuration permits only one type of key pair for an SSL endpoint. You can configure the DLB to use an SSL certificate and a key that are generated using either an RSA or ECDSA algorithm, but not both.
