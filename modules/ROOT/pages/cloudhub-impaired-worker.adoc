...
We should improve our documentation to let customers know that the new workers are provided on a schedule (fixed rate). This way customer would be aware that they should have it in consideration when defining multiple workers. 

the impair instance launch is running at a fixed interval and this is depending on underlying infrastructure, as long as the application has a proper High Availability design (running multiple workers), this shouldn't impact the application.
