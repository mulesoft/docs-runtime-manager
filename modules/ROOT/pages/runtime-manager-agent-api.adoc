...
For the majority of use cases, all you'll need to do is install and configure Runtime Manager agent using xref:installing-and-configuring-runtime-manager-agent.adoc[Install or Update Runtime Manager Agent]

To use Runtime Manager agent for more than managing the communication between xref:index.adoc[Runtime Manager] and Runtime Manager on-premises, you might find the documents in this section helpful:
...
[NOTE]
None of these topics relate to the xref:3.8@mule-management-console::index.adoc[Mule Management Console]. These topics are about Runtime Manager agent that communicates with Runtime Manager with local runtimes.
