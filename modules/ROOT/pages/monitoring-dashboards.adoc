...
The builtin monitoring dashboard gives you in-depth visibility into usage metrics about both your deployed apps and the systems they're deployed to. The dashboard gives you a view into the evolution of metrics about your apps. 
...
To view dashboards, your user must have the Runtime Manager permissions *Read Application* or *Read Servers* for the resource.
...
* *Total* - The maximum amount of memory (in MB) for use by the server. The maximum amount can change or be undefined. A memory allocation can fail if the Java VM attempts to increase the used memory to be greater than committed memory, even if the amount used is below the total.

At a high level, you can think of memory as divided into what’s in the JVM heap and what’s outside of it.
