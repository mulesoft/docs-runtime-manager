...
*CPU Usage* is a percentage value and indicates how much of the server's processing capacity is being used. *Available processors* is an integer value and indicates the number of processors available in the server.
CPU load is the number of tasks which are being executed, or waiting to be executed, by the server's CPU. Therefore, *Load average* is an integer value and indicates the average number of processes that are running or waiting to be assigned CPU time over a given period of time.
