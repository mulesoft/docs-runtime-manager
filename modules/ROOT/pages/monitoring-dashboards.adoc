...
If the selected application is deployed to CloudHub, running on Mule 3.8.7 or later, and you have Anypoint Monitoring enabled for the selected application, you see the embedded xref:monitoring::app-dashboards.adoc[Anypoint Monitoring] dashboard. Click the *Switch back to classic metrics* to switch to the classic view of the app monitoring dashboard. 

If you selected an application deployed to CloudHub and running on a Mule version earlier than 3.8.7, or deployed on-premises, or to Runtime Fabric, you see the classic application monitoring dashboard. Click the link in the blue banner at the top of the page to switch to the embedded Anypoint Monitoring dashboard. 
