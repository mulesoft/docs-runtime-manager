...
* *Routing Type* - Select *Static* or *BGP* as the routing type to use for the VPN. 
. To use *BGP* routing (This type of routing is recommended if your device supports this protocol):
.. Select *BGP*, and enter the following values: +
* *Remote ASN* - Enter the ASN corresponding to your backend under Remote ASN. The default value is 65001.
* *Local ASN* - Assign an ASN to the Mule VPC side under Local ASN. The default value is 64512.
. To use *Static* routing:
.. Select *Static*.
.. Click *Add New Rule*.
.. Enter a Classless Inter-Domain Routing (*CIDR*) address. For example: + ---- 10.0.0.0/8 ----
. Select the type of tunnel configuration for the tunnel used to connect between Anypoint Platform and your network. +
