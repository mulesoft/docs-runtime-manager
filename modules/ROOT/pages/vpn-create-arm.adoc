...
== Access the VPN Page
To access the *VPN* page, you must have either the *CloudHub Network Administrator*, or the *CloudHub Network Viewer* user permission. In addition, assign *Read Application* permissions for the user's default environment (defined under the user's profile). See xref:access-management::users#granting-permissions-and-roles-to-users[Granting Permissions and Roles to Users].

image::vpn-permissions.png[]

[NOTE]
The VPNs page is visible to Anypoint organisations that have VPN entitlements. 
...
. From the menu on the left, click *VPNs*, then click *Create VPN*.
...
+
[NOTE]
All configuration files represent the minimum requirement of IKEv1, AES128, SHA1, and DH Group 2. You may modify the configuration for your VPN endpoint to take advantage of the other supported IPsec Settings.
