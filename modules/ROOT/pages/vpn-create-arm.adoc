...
Static Routes (CIDR)::The subnets in your network to make accessible through the VPN. This is required only if you are using static routing.
...
+
* *To use dynamic routing*, select *BGP*, and then enter the following values:
+
** *Remote ASN*
+
...
* *To use static routing*, select *Static*, and then: 
+
.. In the *CIDR* field, enter a subnet that needs to be accessible via the VPN, for example, `10.0.0.0/8`.
...
+
* *To use Automatic tunnel configuration*, select *Automatic*.
+
...
* *To use Custom tunnel configuration*, select *Custom*, and then enter the values for both tunnels:
