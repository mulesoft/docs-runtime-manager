...
|`${resource}` | The name of the application triggering the alert | ?
|`${severity}` | The severity of the alert. |All
|`${state}` | The xref:managing-deployed-applications#status_states[state] of the target triggering the alert, such as Running or Deployment Failed. | ?
