...
When you are planning and configuring your your CloudHub VPCs, you should make sure you have enough licenses to meet your requirements. 

=== 1:1 Relationship Deployment

In this scenario, you are using a 1:1 relationship, which means that each VPC can connect to a single public IP address at a remote location. 

image::vpc-1-1-deployment.png[1:1 VPC Deployment]

=== Connect a Single VPC to Two Remote Locations
...
=== Connect Two VPCs to Two Remote Locations

If you implement a VPC deployment scenarious where both VPCs connect to two remote locations, four VPC licenses are required, even if you are using the same two public IP addresses. This is because each remote location that needs to be accessible from the Cloudhub VPC requires an additional VPC license.
