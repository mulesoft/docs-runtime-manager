...
*It is recommended to turn off the persistent queue in batch jobs* as it has the below-mentioned limitations with no added benefits. Disabling persistent queue in batch renders better performance. You can disable persistent queue for batch individually by adding a property to the application, "batch.persistent.queue.disable=true".  By default, this property is set to false. When this property is set to true, the application will not use the persistent queue for batch, but the persistent queue will work for other mule components.
