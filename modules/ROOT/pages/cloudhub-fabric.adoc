...
Each application can be deployed with up to 8 workers of any kind, totaling up to 128 vCores. However, you may be limited to fewer vCores than those you need, based on how many are available in your subscription. See xref:deploying-to-cloudhub.adoc#worker-size-and-vcores[Worker Size and vCores] for information about deploying to multiple vCores.
...
Next to *Workers*, select options from the drop-down menus to define the number and type of workers assigned to your application. See xref:deploying-to-cloudhub.adoc#worker-size-and-vcores[Worker Size and vCores] for more information about deploying to multiple workers.
...
For your application to benefit from persistent queuing, implement reliability patterns in your application code, separating individual XA transactions from VM transports. See xref:3.9@mule-runtime::reliability-patterns.adoc[Reliability Patterns] for more information.
