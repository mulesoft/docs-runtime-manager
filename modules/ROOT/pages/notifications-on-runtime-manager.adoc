...
. Download the *CloudHub connector* from the xref:exchange::index.adoc[Anypoint Exchange]
+
...
. You may also want to set up email alerts to inform you when one of these custom events occurs. For setting up these alerts, see xref:alerts-on-runtime-manager.adoc[Alerts on Runtime Manager]. You must create an alert of type *Custom Notification Alert* that matches your custom notification.

See the xref:custom-application-alerts.adoc[Custom Application Alerts] tutorial.
...
* Check out the xref:custom-application-alerts.adoc[Custom Application Alerts] tutorial.
