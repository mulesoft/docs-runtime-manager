...
[%header,cols="30,50,30a"]
|===
|Value |Description | Example
| `--insight` | Store message metadata of every Mule transaction. |
| `--log [level:scope]` | Set logging level (TRACE, DEBUG, INFO, WARN, ERROR, FATAL, or OFF) and scope pair.

To set multiple logging levels, provide multiple `--log` options.| `--log INFO:module1 --log WARN:critModule`
|===
