...
* <<Commands for Managing Anypoint DataGraph>>
...
This command uploads a rest-api, soap-api, http-api, custom, app, template, example, policy, or connector asset using the ID passed in <assetIdentifier>.

Use the following format for the _assetIdentifier_ argument: `([_group_id_]/)_asset_id_/_version_. If _group_id_ is not specified, it defaults to the currently selected organization ID.

[%header,cols="30a,40a,30a"]
|===
| Command | Description |  Example
| `--name` (required) | Asset name .4+<.<|
...
`exchange asset uploadv2 --files.mule-application.jar /..../file-path/mule-app.jar --files.pom /..../file-path/mule-app.pom fa7b266c-3817-4cbb-ae49-7f3cc6c8cd9e/my-mule-app/1.0.0`
    ...
    Required if no file is uploaded. If it is uploaded, type is inferred from the classifier of the file.
    
    Supported Values: `rest-api`, `soap-api`, `http-api`, `raml-fragment`, `custom`, `connector`, `template`, `example`, `policy`, `app`, `extension`, `external-library`
    | `--keywords` | Keywords
    | `--dependencies` | Asset dependencies, separated by ",". |`groupId:assetId:version,groupId2:assetId:version`
    | `--properties` | Asset properties | `properties.mainFile main.yaml`
