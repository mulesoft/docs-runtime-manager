...
|<<exchange asset resource upload>> | Upload a resource to an asset portal
...
|<<exchange asset resource delete>> | Delete resource from the asset portal as draft and then publish that draft
|===
...
| `--dependencies` | Asset dependencies, separated by "," |
...
This command uploads a resource `<filepath>` to an asset page describes in `<assetIdentifier>`.

Argument `assetIdentifier` should be formatted as follows: `([group_id]/)<asset_id>/<version>`. +
If `group_id` is not specified, it defaults to the currently selected Organization ID.

Supported file extensions: jpeg, jpg, jpe, gif, bmp, png, webp, ico, svg, tiff, tif.

The successful output command will be a markdown codesnippet.
You can use it on any page of your `<assetIdentifier>` asset.
...
This command lists published resources for `<assetIdentifier>` in the asset portal

Argument `assetIdentifier` should be formatted as follows: `([group_id]/)<asset_id>/<version>`. +
If `group_id` is not specified, it defaults to the currently selected Organization ID.

You can use `--draft` option to list draft resources in the asset portal.
...
This command downloads the specified published resource `<resourcePath>` from the asset portal `<assetIdentifier>` to `<filePath>`

Argument `<assetIdentifier>` should be formatted as follows: `([group_id]/)<asset_id>/<version>`. +
If `<group_id>` is not specified, it defaults to the currently selected Organization ID.

Argument `<resourcePath>` should be a resource path of a publicated resource. You can list publicated resources with `list` command (exchange asset resource list)
...
This command delete `<resourcePath>` from the asset `<assetIdentifier>` in asset portal as draft and then publish that draft
...
For more configuration information, see xref:lb-ssl-endpoints.adoc[Configure SSL Endpoints and Certificates].
