...
[NOTE]
This command will publish any active draft as part of the operation.
...
This command uploads a resource from `<filepath>` to an asset portal described in `<assetIdentifier>`.
...
This command deletes the resource `<resourcePath>` from the asset `<assetIdentifier>` by publishing a new draft portal where the resource has been deleted.
