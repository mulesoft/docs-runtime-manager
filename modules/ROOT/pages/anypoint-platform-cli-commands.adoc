...
|<<exchange asset uploadv2>> | Uploads an Exchange asset
...
Upload a rest-api, soap-api, http-api, custom, app, template, example, policy and connector asset. Basic example to upload a raml asset: exchange asset uploadv2 --name "Raml Asset" --description "RAML" --properties.apiVersion v1 --properties.mainFile api.raml --files.raml.zip /..../file-path/raml.zip raml-asset/1.0.0. Describing the asset.
