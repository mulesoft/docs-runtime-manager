...
|429 a|The operation was unsuccessful due to rate limit.

* Logs query('v2/applications/{domain}/logs','v2/applications/{domain}/deployments/{deploymentId}/logs','v2/applications/{domain}/instances/{instanceId}/logs') limit is one call per second on each application.
* Logs download(v2/applications/{domain}/instances/{instanceId}/log-file) limit is one call per minute on each application.
