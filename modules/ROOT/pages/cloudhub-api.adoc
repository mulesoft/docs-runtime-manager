...
The CloudHub management API enforces usage limits.
These limits don't apply to CloudHub workers.


=== Global Rate Limits for CloudHub APIs 

The following table lists the rate limits for all CloudHub APIs except the items listed in <<Exceptions to the Global Rate Limits>>. 

When the app reaches the rate limit, CloudHub returns a 503 status code.

[%header,cols="20,20,20,10"]
|===
|Endpoints |Control Plane | Rate Limit Per IP Address | Status Code
.3+|All except specified endpoints|U.S. .2+|75 requests per second  .3+| `503`
|EU 
|Government Cloud|25 requests per second |
|===


=== Exceptions to the Global Rate Limits

The following table lists the exceptions to the <<Global Rate Limits for CloudHub APIs>>. 

When the app reaches the rate limit, CloudHub returns the status code.

[%header,cols="20,20,20,10"]
|===
|Endpoints|Control Plane | Rate Limit Per IP Address| Status Code 
4+|*Endpoints under* `/api`
.3+|`/notifications`|U.S. .2+|75 requests per minute  .3+| `503`
|EU
|Government Cloud|25 requests per minute
