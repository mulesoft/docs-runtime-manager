...
The following rate limits apply to apps deployed to CloudHub:

[%header,cols="30a,50a"]
|===
|Control Plane | Rate Limit
|U.S.|75 requests per second per IP address
|EU|25 requests per second per IP address
|Government Cloud|25 requests per second per IP address
|===

When an app reaches the limit, CloudHub returns error code `503`.

The following endpoints have a limit of one call per second for each application:
...
When an endpoint reaches the limit, CloudHub returns error code `500`.

The following endpoint has a limit of one call per minute for each application:
...
|503 |The operation was unsuccessful due to reaching the rate limit.
|===
