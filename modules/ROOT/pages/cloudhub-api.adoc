...
|429 |The operation was unsuccessful due to rate lmit. 
...
For a 429 status code. The folling rate limits are in effect 
----
1.Logs query is allowled one call per second on each application
2.Logs download is allowled one call per minute on each application
----
