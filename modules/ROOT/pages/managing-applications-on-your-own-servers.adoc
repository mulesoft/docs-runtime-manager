...
Runtime Manager doesn't support downloading the application archive (ZIP or JAR file) from the application *Settings* page for an app deployed to an on-premises server.
However, you can download the app archive using an ARM REST Services API call or the Anypoint command-line interface.
