...
[NOTE]
In Runtime Manager, you can see and manage only applications deployed through Runtime Manager.
If you manually deploy applications on your on-premise servers, they aren't visible in Runtime Manager as the applications are not auto-discovered.
For this reason, MuleSoft does not recommend manual deployment.
