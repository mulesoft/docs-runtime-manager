...
If a load balancer that requires an update is restarted, the update occurs automatically. Conditions when the load balancers restart include recovery from an impaired state and when manually restarted using Runtime Manager or the API.
