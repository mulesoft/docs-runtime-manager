...
While persistent queues are highly available within a region, they might not accessible if the region or part of the region is unavailable (usually a few seconds or minutes), which could result in some data loss.
After the region is available again, CloudHub reinstantiates the queues.
