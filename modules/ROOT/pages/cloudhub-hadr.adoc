...
If an application uses a single worker, when the availability zone is unavailable, you must manually restart the application after the zone becomes available. You can set up `status.mulesoft.com` to receive alerts when a failure occurs in an availability zone or region.
