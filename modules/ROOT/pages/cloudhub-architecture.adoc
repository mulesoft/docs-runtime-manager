...
With xref:cloudhub-fabric.adoc[clustering], you can also scale your applications horizontally by adding multiple workers and load distribution over queues. For more information about scaling workers, see xref:cloudhub-fabric.adoc[Worker Scaleout]. For more information about using persistent queues on CloudHub, see xref:cloudhub-fabric.adoc[Persistent Queues].

[TIP]
You can xref:worker-monitoring.adoc[monitor your workers] to verify that they are all operating properly, and you can enable automatic restarts if one or more workers is not.
...
With xref:cloudhub-fabric.adoc[Clustering], you can add multiple workers to your application to make it horizontally scale. This also adds additional reliability. CloudHub automatically distributes multiple workers for the same application across two or more datacenters for maximum reliability.
