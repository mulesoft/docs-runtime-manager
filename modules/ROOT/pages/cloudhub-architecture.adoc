...
[cols="10a,45a,45a"]
|===
|  |<<Integration Applications>> |The applications that you create and deploy to CloudHub to perform integration logic for your business
|image:blue-1.png[] |<<Runtime Management Console>> |The user interface that enables you to deploy and monitor integrations, and configure your account
...
For more about creating and deploying integration applications, see xref:cloudhub.adoc[CloudHub] and xref:deploying-to-cloudhub.adoc[Deploy to CloudHub].
...
Applications on CloudHub are run by one or more instances of Mule runtime engine, called *workers*.
...
If your application requires more storage (for example, verbose logging), use a worker with two or more vCores, enabling the application to access additional storage in `/tmp`.

Only running applications count toward workers usage.

If your applications require more vCores than you are allocated, CloudHub allows you to create the application, but you cannot start it until the additional vCores become available.
To increase vCore allocation, stop another application or contact your account manager to increase the vCore allocation in your subscription.
...
You can scale your applications horizontally by adding multiple workers and enabling persistent queues to distribute workload across workers.
...
For more information about deploying applications to different regions, refer to xref:deploying-to-cloudhub.adoc[Deploy to CloudHub]. For more information about CloudHub security and compliance, download the https://www.mulesoft.com/lp/whitepaper/saas/cloud-security[Anypoint Cloud Security & Compliance] whitepaper.
...
Clustering provides scalability, workload distribution, and added reliability to applications on CloudHub. These capabilities are powered by the scalable load-balancing service,
