...
Manage CloudHub with the Runtime Manager console in the Anypoint Platform. You can also xref:deploying-to-cloudhub.adoc[deploy] to it directly from Anypoint Studio, via the xref:cloudhub-api.adoc[CloudHub API] or via the xref:anypoint-platform-cli.adoc[Anypoint Platform Command Line Interface (Anypoint CLI)].

[NOTE]
====
See xref:deployment-strategies.adoc[Deployment Strategies] for a better understanding of the different possible deployment scenarios, including on-premises and Anypoint Platform Private Cloud Edition (Anypoint PCE).
====
...
The memory capacity and processing power of a worker depends on how you configure them at the application level. 
...
In addition to less memory available, workers that use a fraction of a vCore are allocated limited CPU and IO and are generally only appropriate for smaller workloads.


[IMPORTANT]
The metaspace limit for apps deployed to CloudHub is 256 MB, regardless of the worker size. Increasing worker size doesn't change the metaspace limit.
