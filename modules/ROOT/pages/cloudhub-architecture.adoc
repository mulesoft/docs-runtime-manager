...
The metaspace limit for apps deployed to CloudHub is currently 256 MB, regardless of the worker size. MetaspaceSize property is also set so that the gc starts to collect metaspace once a 128 MB threshold is reached.
