...
When developing an application that needs to access resources from inside your firewall, you can configure a xref:virtual-private-cloud.adoc[Virtual Private Cloud (VPC)] to host your xref:cloudhub-architecture.adoc#cloudhub-workers[Cloudhub Workers] and configure an IPSec tunnel to access your applications.
