...
Instead, it returns the value as set internally without throwing an error or exception.

Applications can reference reserved properties, for example, to retrieve the application name or deployment region.
You can use the `${csorganization.id}` and `${environment.id}` properties to retrieve those values if your app makes Anypoint Platform API calls.
You can use the `${worker.id}` to identify each worker in a multi-worker application.

--
[NOTE]
The `${java.version}` property value doesn't include the point release for the JDK version.
Additionally, CloudHub runtime release updates might upgrade the JDK.
--
...
|`${java.version}`|JVM version|1.8.0_141
...
|`${host}`|Used internally for local IP address of a worker|`10.40.122.33`
|`${anypoint.url}`|Used internally as the base Anypoint Platform URL|US: `\https://anypoint.mulesoft.com`

GovCloud: `\https://gov.anypoint.mulesoft.com`
|===

--
[NOTE]
For propriety reasons, MuleSoft doesn't make public all internal, reserved properties.
Additionally, CloudHub runtime release updates might change the reserved properties.
--
