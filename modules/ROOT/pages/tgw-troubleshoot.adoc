...
See xref:tgw-manage-arm.adoc#remove-a-transit-gateway-from-anypoint-platform[Remove a Transit Gateway from Anypoint Platform] and xref:tgw-add-arm.adoc#attach-a-vpc-to-the-transit-gateway[Attach a VPC to the Transit Gateway].
...
If the status for the transit gateway is `Unknown`, click the *View on AWS* button on the *Transit Gateways* page to resolve the issue on AWS:

_update screenshot_ and move to context?
