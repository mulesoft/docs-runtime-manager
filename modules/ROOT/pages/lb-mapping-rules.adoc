...
The load balancer configuration is defined by a list of mapping rules which describe how an input URL should be translated into calls to different CloudHub apps.
...
When creating a simple matching rule, one input address is matched to the defined output: the endpoint of one of your applications.
...
The input URL is described using a URI parameter which can be specified by the user. The `URI` is a string or pattern that describes the input URI.
