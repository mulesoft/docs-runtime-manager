...
Given that the external load balancer domain name depends on the unique name you assign to it, in the following mapping rule examples, assume that the load balancer in these examples is `lb-demo`.
...
** `dev-app`
** `qa-app`
* An SSL endpoint with the Subject Alternative Names:
** `dev.example.com`
** `qa.example.com`
* Mapping rule:
+
...
Every rule must have a priority defined. It is highly recommended to pay attention to each rules’ order when creating them. Multiple rules might override each other.
