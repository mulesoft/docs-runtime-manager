...
+
The server name is unique across environments.
If the same organization exists in different environments, the server name can be the same in those organizations.
+
Runtime Manager generates the `amc_setup` command, including the `_registration token_` and the server name you specified, for you to use to register Mule in your environment:
+
...
+
This button only appears if the server name you specify is valid.
...
After the script completes successfully, your server appears on the *Servers* tab of Runtime Manager with a status of Created.
