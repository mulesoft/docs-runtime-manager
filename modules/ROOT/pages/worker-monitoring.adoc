...
+
For readiness assessment to work when tracked endpoints are defined in your application, ensure that all tracked endpoints are correctly linked to the corresponding API in API Manager and that all policies are correctly applied to the tracked API. If a single custom policy of a particular tracked API is not applied, the whole deployment will be considered unsuccessful.
+
If the deployment is not deemed ready within 5 minutes, it is considered a failed deployment, and the logs print an error message.

[NOTE]
The health check endpoint is only available for 4.2.1 runtime version and above.
