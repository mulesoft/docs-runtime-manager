...
+
Every 30 seconds, CloudHub checks the health check endpoint.
+
If CloudHub detects that the health check endpoint fails three times in a row, the logs print a warning.
