...
For applications running on runtime 3.8.x and below, upon deployment, CloudHub inserts a monitoring ping flow into your application. When the application starts, the logs display a message to indicate that monitoring is enabled and that the ping flow has been injected into your application.
...
*Mule Runtime 3.9.x and above*

Every thirty seconds, CloudHub checks the PID of the application. If CloudHub detects the application process is not running three times in a row, the Logs print a warning.

In addition, CloudHub triggers a notification to the console to alert you that a worker for any application stopped responding.
