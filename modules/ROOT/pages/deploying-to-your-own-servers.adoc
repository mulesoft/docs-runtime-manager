...
. xref:mule-runtime::mule-standalone.adoc[Download and install Mule].
. xref:servers-create.adoc[Add a server to Runtime Manager].

You can deploy a Mule application to a local server using:

* xref:deployment-strategies.adoc#cloud-console-on-premises-deployment[Runtime Manager cloud console]
...
== Deploy an Application from Anypoint Platform

. Sign in to https://anypoint.mulesoft.com[Anypoint Platform].
...
== Deploy an Application from a Sandbox Environment
