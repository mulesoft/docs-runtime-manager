...
Deploying on-premises (both via the cloud and the on-premises console) offers high availability capabilities through creating xref:server-group-about.adoc[Server Groups] or xref:cluster-about.adoc[Clusters]. Clustered Mule instances have xref:3.8@mule-runtime::mule-high-availability-ha-clusters.adoc[distributed shared memory]. This shared memory is used to provide persistent VM queues, transactions, and cluster-wide data storage.
