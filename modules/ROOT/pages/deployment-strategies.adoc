...
Unless you choose to configure it otherwise, no application data flows to the cloud, only metadata pushed by the Mule Runtime agent in each Mule runtime engine.
The agent monitors and controls Mule by calling APIs from external systems and publishing its own data (in this case) to the control plane.
If you wish, you can configure the agent to push IDs, final average numbers, or any data you find useful for monitoring or keeping control of applications. 
