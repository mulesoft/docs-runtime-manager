...
|xref:deploying-to-cloudhub.adoc[CloudHub]
...
If the content doesn't apply to a deployment strategy, the icon appears grayed out: image:logo-cloud-disabled.png[link="/runtime-manager/deploying-to-cloudhub", title="CloudHub"]

In xref:studio::index.adoc[Anypoint Studio], clicking the *Play* icon (or right-clicking project on the *Package Explorer* and selecting *Run As* > *Mule Application*), deploys your application to an embedded test server that is built into Studio. This server is not meant for production deployment and uptime restrictions apply. Ultimately you deploy your application into one of the deployment options described below. All of these are supported by the Runtime Manager.
...
=== Runtime Manager to CloudHub Deployment
...
To make a server visible to Runtime Manager, you must first register it by running an instruction on the server's command line, see See xref:servers-create.adoc[Create a Server in Runtime Manager]. You then have the flexibility to group servers into xref:server-group-about.adoc[Server Groups] or xref:cluster-about.adoc[Clusters] to make them a <<High Availability>> deployment target. You can then choose to xref:deploying-to-your-own-servers.adoc[deploy your applications] to either a server, server group or cluster.
...
| You must use the xref:mule-runtime::scheduler-concept.adoc[Scheduler Endpoint] element in your flows to schedule tasks.

| CloudHub has its own preconfigured default xref:managing-application-data-with-object-stores.adoc[Object Store] you can reference. To use it, simply add an  xref:connectors::object-store/object-store-connector.adoc[Object Store connector] and set its 'config_ref' to point to the default CloudHub Object Store.
| To use xref:connectors::object-store/object-store-connector.adoc[Object Stores] you must configure your own database to store data.
|===
...
You can also hide these public URLs through your DNS name servers. For example you could create A records to route requests to `myapplication.mycompany.com` from `myapplication.cloudhub.io`.

Alternatively, CloudHub includes optional xref:cloudhub-dedicated-load-balancer.adoc[Dedicated Load Balancers] that you can add to a Virtual Private Cloud (VPC) for handling the DNS and load balancing of your applications within the VPC, and to define custom firewall rules within your VPC, such as to expose other inbound TCP ports besides ports 80/443 and 8081/8082. Through this, you can apply vanity domains and host your applications under any URL you choose.

image::infrastructure-cloud-vpc.png[vpc]

To use the load balancer, your application must use specific ports that CloudHub allocates for your HTTP and HTTPS endpoints. See xref:developing-applications-for-cloudhub.adoc[Developing Applications for CloudHub] for more details.
...
CloudHub provides high availability through xref:cloudhub-fabric.adoc[clustering]. Clustering provides a combination of load balancing, persistent message queues, and horizontal scale out. It also actively monitors services and workers for problems. For example, in the case of hardware failure, CloudHub automatically migrates the application to a different worker using xref:managing-applications-on-cloudhub.adoc[CloudHub zero-downtime updates], minimizing downtime.

Deploying on-premises (both via the cloud and the on-premises console) offers high availability capabilities through creating xref:server-group-about.adoc[Server Groups] or xref:cluster-about.adoc[Clusters]. Clustered Mule instances have xref:mule-runtime::mule-high-availability-ha-clusters.adoc[distributed shared memory]. This shared memory is used to provide persistent VM queues, transactions, and cluster-wide data storage.
...
[NOTE]
xref:mule-runtime::mule-high-availability-ha-clusters.adoc#clustering-for-high-performance[Clustering for High Performance] is not supported on Pivotal Cloud Foundry.
...
Deployments on-premises require that you set up your own object store, see  xref:connectors::object-store/object-store-connector.adoc[Mule object stores].
...
For apps that you deploy to servers on-premises, through any modality, this is not an option. You can achieve the same by including the xref:mule-runtime::scheduler-concept[Scheduler] element in the flows of your application.
...
If you want to deploy the same Mule application via various different deployment strategies, such as to an <<Cloud Console On-Premises Deployment, on-premises server>> and <<Cloud Console CloudHub Deployment, CloudHub>>, abstract some parameters of the application to xref:mule-runtime::intro-packaging.adoc[application properties] that you can set with different values in each use case, without needing to alter the actual application.
...
* Deploy to a xref:mule-runtime::deploying.adoc[standalone Mule runtime].
* Deploy via the xref:mule-management-console::index.adoc[Mule Management Console].
