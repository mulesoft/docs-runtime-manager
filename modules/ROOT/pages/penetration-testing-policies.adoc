...
To report security vulnerability findings, please follow the https://help.salesforce.com/articleView?id=000320207&type=1&mode=1[Security Vulnerability Finding Submittal Guide]. Security vulnerability reports that do not appropriately follow the required steps will be rejected.

Please, note that Salesforce does not address any security vulnerabilities that fall under the following categories:

* Scan output from automated vulnerability scanning tools without any exploits or POCs.
* Security bug without a valid exploit or proof of concept.
* Vulnerabilities identified in an old or deprecated version of Mule runtime.
* Vulnerability induced via code customization made by your company.
