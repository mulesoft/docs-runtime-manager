...
== Encoding UTF-8 Characters in DLB URLs

You can specify to encode UTF-8 characters in DLB URLs: 

* If you select *Keep URL encoding*, all UTF-8 characters in the DLB URL are encoded.
+
The change takes place after restarting the DLB.
* If you deselect *Keep URL encoding*, only the `%20` and `%23` characters in the DLB URL are encoded.
