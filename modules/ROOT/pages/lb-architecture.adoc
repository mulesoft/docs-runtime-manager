...
For example, supose you define a create a load balancer named *lb-name*. Then the dedicated load Balancer exposes an external domain name that resolves to two public IP addresses which are accessible from outside your CloudHub Anypoint VPC network.
