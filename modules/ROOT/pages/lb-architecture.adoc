...
A CloudHub dedicated load balancer is assigned to an Anypoint VPC. The dedicated load balancer then routes traffic to that Anypoint VPC within the service region of the Anypoint VPC.

For a Mule application to be managed by the dedicated load balancer, the Mule application must be deployed to the Anypoint VPC for the dedicated load balancer.

The CloudHub dedicated load balancer has an internal domain name to be used by applications and clients within the Anypoint VPC.
...
You can use a CloudHub dedicated load balancer to route both internal and external traffic and route requests for multiple deployed Mule applications.
