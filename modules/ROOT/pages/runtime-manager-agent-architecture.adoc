...
The Runtime Manager agent is a Mule plugin that exposes the Mule JAVA API as a service, allowing users to manipulate and monitor Mule instances from external systems.
...
During installation, you also have the option to configure the Runtime Manager agent using a quick-start script. For details, see xref:installing-and-configuring-runtime-manager-agent.adoc[installing and configuring Runtime Manager agent]
