...
[IMPORTANT]
If the applicaiton is using Static IPs, an update may not be completely zero down time. During the very short time period where the Static IP transfers from the old to the new version of the application, the application will not be responsive. Please ensure the client has retry relevant retry mechanisms to account for this.
