...
You can use the following migration script to update `policy_violation_policy_id` data type in MySQL:
...
. Copy the `mysql-connector-java-_VERSION_-bin.jar` file to `{MULE_HOME}/plugins/mule-agent-plugin/lib/modules`.
...
If you are migrating from a Runtime Manager agent version earlier than 2.3.0,
you must update the schema. 
You can use the following migration script to update `policy_violation_policy_id` data type in Oracle 11g XE:
...
If you are migrating from a Runtime Manager agent version earlier than 2.3.0,
you must update the schema. 
You can use the following migration script to update `policy_violation_policy_id` data type in Microsoft SQL Server 2014:
...
. Extract the `sqljdbc_4_%version%.tar.gz` file.
. Copy the `sqljdbc4%version%_.jar` file to `{MULE_HOME}/plugins/mule-agent-plugin/lib/modules`.
. Modify the file `{MULE_HOME}/conf/mule-agent.yml` to include the following:
+
[source,yaml,linenums]
----
