...
The *trackingLevel* value specifies one of the four tracking levels (NONE, BE, TRACKING, DEBUG) described in the previous table. This tracking level is applied to every flow in the Mule application.
...
The `globalTrackingLevel` is set to `OFF`, which is the default. You could remove the `globalTrackingLevel` line and there will be no change to the Runtime Manager Agent's configuration.
 ...
 Tracking level `TRACKING` is set for the `mule-flights` application, but tracking level `DEBUG` is set for the `purchaseFlight` flow, and the r`eplayLevel` is set to `ALL`.
