...
.. In the *Create Notificaton* properties window, enter the app name in the *Domain* field and the notification in the *Message* field:
+
...
When you trigger the alert, the recipients you specified receive an email with the custom message you added to the app.

You can also use the CloudHub Connector inside an `<error-handler` component to send notifications when errors happen.
...
You can use the CloudHub Connector inside an error handler to send notifications when errors happen. 
...
<error-handler>
  <on-error-continue>
     <cloudhub:create-notification domain="osv2-demo2" doc:name="Create Notification" config-ref="CloudHub_Config" priority="INFO" transactionId="uuid()">
	   <cloudhub:message ><![CDATA[#["Error processing transaction"]]]></cloudhub:message>
