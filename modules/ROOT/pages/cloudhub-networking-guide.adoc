...
After a static IP has been allocated for your application, the IP address is visible in the *Static IPs* tab in the application settings. For more information about static IPs, see xref:deploying-to-cloudhub.adoc#static-ips-tab[Deploy to CloudHub].
...
If you need to whitelist the IP addresses for the communication between the xref:runtime-manager-agent.adoc[Runtime Manager Agent] and the Runtime Manager console, see xref:rtm-agent-whitelists.adoc[Ports, IP Addresses, and Hostnames to Whitelist].
...
https://anypoint.mulesoft.com[Anypoint Platform] for the MuleSoft Government Cloud
covers the following regions.

[%header,cols="2*a"]
|===
| Regions in U.S. Control Plane | DNS Record
| US Gov West | `myapp.us-gov-west-1`
