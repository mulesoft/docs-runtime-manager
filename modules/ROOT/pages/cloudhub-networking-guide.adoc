...
You can set your own xref:cloudhub-dedicated-load-balancer.adoc[dedicated load balancer] through the xref:anypoint-cli::index.adoc[Anypoint Platform Command-Line Interface (Anypoint Platform CLI)] by configuring your own xref:virtual-private-cloud.adoc[Anypoint VPC].
...
. Create an xref:lb-allowlists.adoc[allowlist] in your dedicated load balancer with the IP addresses you want to authorize. You can only do this using the xref:anypoint-cli::cloudhub-dlb.adoc#cloudhub-load-balancer-whitelist-add[`cloudhub load-balancer whitelist add`] command from Anypoint Platform CLI.
