...
If you need to allowlist the IP addresses for the communication between the xref:runtime-manager-agent.adoc[Runtime Manager Agent] and the Runtime Manager console, see xref:rtm-agent-allowlists.adoc[Ports, IP Addresses, and Hostnames to Allowlist].
...
. Create an xref:lb-allowlists.adoc[allowlist] in your dedicated load balancer with the IP addresses you want to authorize. You can only do this using the xref:anypoint-platform-cli.adoc#cloudhub-load-balancer-whitelist-add[`cloudhub load-balancer whitelist add`] command from Anypoint Platform CLI.
