...
Deploying to a region also affects your internal and external worker DNS address. For example, if you are using the US control plan and deploy to the Ireland region, the DNS records for internal and external IP addresses are `mule-worker-myapp.ir-e1.cloudhub.io` and `mule-worker-internal-myapp.ir-e1.cloudhub.io`.
