directory= Install and Configure the Runtime Manager Agent
...
You can configure Runtime Manager agent to establish a TLS connection using the `-S` argument on the installation command line. This option enables a TLS channel for REST communications only.
...
You must provide keystore (public/private key pair) and truststore files in JKS format.

|===

Once you select the secure connection channel mode, you see the following menu:
...
* *Keystore location*
+
Specifies the location of the keystore file to encrypt the communication channel. The keystore file must be in JKS format. (Required)
* *Truststore location*
+
Specifies the location of the truststore file to accept incoming requests from the administration console. The truststore must be in JKS format and must not have a password.
* *Keystore Password*
+
Provides the password to read the keystore. The agent uses the password to open the keystore.
* *Keystore Alias*
+
...
+
[NOTE]
In Runtime Manager agent version 1.10.0 and later, the name of the default truststore was `anypoint-truststore.jks`. In earlier versions of Runtime Manager agent, the name was `truststore.jks`.
...
. Rename the newly created `cacerts.jks` file as `anypoint-truststore.jks`. This file is your new truststore for Runtime Manager agent as well as Anypoint Platform connectivity.
+
----
...
. Start Mule runtime engine, and verify the Runtime Manager agent REST interface starts up successfully and that it is still connected to Runtime Manager in Anypoint Platform.
