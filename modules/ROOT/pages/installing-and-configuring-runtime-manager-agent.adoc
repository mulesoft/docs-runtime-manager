...
The Runtime Manager Agent is installed into a Mule runtime engine (Mule), and is used to register the Mule with Runtime Manager. After the Mule runtime engine is registered, it can be managed by Runtime Manager within the specific environment and Anypoint Platform business group in which it was registered. A Mule can't be registered with multiple Runtime Manager business groups or environments.
...
You can configure Runtime Manager agent to establish a TLS connection by using the `-S` argument on the installation command line. This option enables a TLS channel for REST communications only.
...
After you select the secure connection channel mode, you see the following menu:
...
+
For example, if you ran the previous commands from the `/security` folder, enter these values:
+
[source,console,linenums]
----
-> Mule Agent Unpacked
...
+
[NOTE]
In Runtime Manager agent version 1.10.0 and later, the name of the default truststore is `anypoint-truststore.jks`. In earlier versions of Runtime Manager agent, the name is `truststore.jks`.
...
. Rename the newly created `cacerts.jks` file to `anypoint-truststore.jks`. This file is your new truststore for Runtime Manager agent as well as Anypoint Platform connectivity.
+
----
...
[%header,cols="3*a"]
|===
|*US*|*runtime-manager.anypoint.mulesoft.com* | TBA
|*US*|*runtime-manager.anypoint.mulesoft.com* | TBA
|*US*|*runtime-manager.anypoint.mulesoft.com* | TBA
|*US*|*runtime-manager.anypoint.mulesoft.com* | TBA
|===
...
|===
|Region |Name |IP Address
|*EU*|*mule-manager.anypoint.mulesoft.com* |18.195.19.18
|*EU*|*mule-manager.anypoint.mulesoft.com* |18.194.245.32
|===
