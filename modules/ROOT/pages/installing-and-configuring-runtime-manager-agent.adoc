...
* xref:<<encrypt_password,Encrypt passwords in the `mule-agent.yml` file>>.
...
For information, see <<Update the Master Password>>.
...
with Runtime Manager, you can encrypt these passwords later in the `mule-agent.yml` file:

keystorePassword::
Keystore password used to access the `mule-agent.jks` file
keystoreAliasPassword:: 
Keystore alias password used to access the `mule-agent.jks` file
password::
Password used for authenticating with a proxy server
