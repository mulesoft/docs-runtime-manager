...
use the `mule-agent-backup.zip` file to restore the previous version.
The `mule-agent-backup.zip` file includes libraries, configuration files, and compiled classes for the agent.

To roll back the agent:

. Stop Mule or the API gateway runtime.
. Run the following commands to restore the previous version of the agent from the `mule-agent-backup.zip` file:
+
