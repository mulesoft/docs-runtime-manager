...
`--decrypt`
|
|Decrypts the passwords in an existing `mule-agent.yml` file, for example, before changing the master password.

Use this option alone to decrypt passwords.
For information, see <<Update the Master Password>> and <<Display Passwords in mule-agent.yml in Plain Text>>.
...
+
`$MULE_HOME/bin/amc_setup --decrypt`
. Set the `AGENT_VAR_master_password` environment variable to the new master password:
+
