...
. Post *Deployments* on the *Application Manager API*, passing the organization's 'id', the environment's 'id', 'applicationName', 'labels', 'target' deployment settings (including the target id retrieved in the in step 1 of this section), the Exchange referenfe of the published Mule application 'application' to deploy, the application 'configuration' and optionally the application 'id'.
