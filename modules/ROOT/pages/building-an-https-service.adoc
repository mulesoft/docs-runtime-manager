...
CloudHub supports deploying HTTPS- and SSL-based services so you ensure your data confidentiality. 
These configuration steps shown are meant for deploying your Mule app to CloudHub.
...
+
Take advantage of Anypoint API Manager integration with the secrets manager. 
+
...
For detailed information about the Anypoint Connector for HTTP, see the xref:3.8@mule-runtime::http-connector.adoc[HTTP Connector for Mule 3] documentation.

To modify your existing service to use HTTPS:

. Generate a `keystore` file.
+
You can create a `keystore` file using the JDK `keytool` utility on the command line, for example:
+
...
. Configure an HTTP Connector in your Mule app:
+
[source,xml,linenums]
----
...
...
. Include a `config-ref` reference to the HTTPS global listener configuration, for example:
+
[source,xml,linenums]
----
...
. Include a `config-ref` reference to the HTTPS global request configuration, for example:
+
[source,xml,linenums]
----
<flow name="httpsserviceFlow">
...
. Add the `keystore` values: `path`, `password`, and `keyPassword`, for example:
+
[source,xml,linenums]
----
