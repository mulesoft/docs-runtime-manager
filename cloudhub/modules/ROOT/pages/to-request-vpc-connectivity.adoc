...
== Configure Several Connections per VPC

Each MuleSoft VPC supports up to 48 active peering connections. A single VPC may also support multiple VPN connections, depending on the VPN capacity allocated to your account.
...
[NOTE]
Each VPN connection requires a VPN license. Each peering connection is included in the VPC license.

== Share Connections Between Different VPCs

Each VPC is independent, and you must create connections on a per-VPC basis. However, you can use a Transit Gateway to share Direct Connect, VPC Peering, and AWS Site-to-Site VPN connections.
