...
* xref:anypoint-code-builder::int-deploy-mule-apps.adoc[Anypoint Code Builder]
...
If you create and test an application in a sandbox environment, you can move it to the
production environment for deployment.
To move applications between environments, you need the following permissions:

* At the business group level:
** Exchange Viewer
** Read Runtime Fabrics
* In the target environment (where you are deploying the application):
** Create Applications
** Download Applications
** Read Applications
* In the source environment (from where the application is fetched):
** Download Applications
** Read Applications

For information about configuring permissions, see xref:access-management::teams.adoc[Teams].
...
+
include::partial$mule-runtime-eos-note.adoc[]
...
[NOTE]
Autoscaling isn't available on CloudHub for Usage-based Pricing (UBP) organizations.
...
For more information about adding more static IPs, see https://help.salesforce.com/s/articleView?id=001121221&type=1[How to Request Additional Static IPs for your Anypoint Organization].
