...
[NOTE]
When you add a new route, the platform ensures it doesn't conflict with any existing routes in the VPC, whether they're static or propagated from a BGP connection. If the new route is a duplicate, either remove the existing route or split the new route into smaller ones to avoid any collisions.
...
In the route tables, added routes show *Transit Gateway* as the value in the *Next Hop* column. 

Specific routes attached to the transit gateway show under *Transit Gateway Routes*. Other routes in the VPC show under *VPC Routes*.
