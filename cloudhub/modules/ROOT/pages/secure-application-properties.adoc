...
[NOTE]
====
This page describes safely hidden application properties for CloudHub (1.0) and hybrid deployments.

On CloudHub 2.0, secure-property masking is configured in Runtime Manager on the *Properties* tab (property protection), not through `mule-artifact.json` or config-file masking. Masked values aren't viewable or retrievable after being set and are encrypted in the Anypoint Security secrets manager. Runtime Manager properties override archive-bundled properties of the same name.

On CloudHub 2.0, configuring masking through `mule-artifact.json` or config-file masking isn't the recommended practice. For information, see xref:cloudhub-2::ch2-protect-app-props.adoc[Protecting App Property Values].
====
...
Even if you edit your application file to remove the `secureProperties` definitions from your `mule-artifact.json` file and then upload that edited application file to CloudHub, CloudHub keeps the invisibility of the properties previously flagged as safely hidden. Any property added as secure at any time remains secure even after it is deleted from the `mule-artifact.json` file.
...
* xref:cloudhub-2::ch2-protect-app-props.adoc[Protecting App Property Values (CloudHub 2.0)]
...
* https://help.salesforce.com/s/articleView?id=001117183&type=1[Hide Configuration Properties in Hybrid Deployments]
