...
On CloudHub 2.0, secure-property masking is configured in Runtime Manager on the *Properties* tab (property protection). Masked values aren't viewable or retrievable after being set and are encrypted in the Anypoint Security Secrets Manager. Runtime Manager properties override archive-bundled properties of the same name, so configuring value masking through `mule-artifact.json` or a configuration file doesn't take effect on CloudHub 2.0. For more information, see xref:cloudhub-2::ch2-protect-app-props.adoc[].
====
