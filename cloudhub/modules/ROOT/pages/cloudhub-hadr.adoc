...
|Customer |Define and implement a cross-region DR strategy for primary and backup regions
|Customer |Decide when to trigger regional failover—for example, based on health checks or business criteria
|Customer |Configure Global Server Load Balancing (GSLB) or a dedicated load balancer (DLB) and routing rules to direct traffic to a backup region during a disaster
|Customer |Application-level failover: deploy and maintain applications in more than one region when you need cross-region DR
|Customer |Replicate and back up external data stores such as databases, object stores, and other systems that your applications use across regions
|Customer |Set up Anypoint VPC in each region when you need network connectivity there for DR
|===
