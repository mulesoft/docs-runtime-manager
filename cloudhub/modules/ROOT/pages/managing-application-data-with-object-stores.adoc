...
* xref:object-store::osv2-faq.adoc[Object Store v2] expands the number of cloud object store regions and object store capabilities.
====

[IMPORTANT]
====
CloudHub clears local disk storage when an app restarts or redeploys. Because Object Store v1 writes to local disk, data isn't preserved, even if you configure `persistent="true"`.

In Anypoint Studio and standalone Mule runtimes, the local file system persists, so object stores retain data across restarts. CloudHub workers use volatile disk storage.
