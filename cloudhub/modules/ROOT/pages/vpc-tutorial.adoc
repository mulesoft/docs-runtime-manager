...
[NOTE]
If a new or updated VPC (VPC A) is associated with an environment already linked to another VPC (VPC B) in the same region, VPC B loses the environment, and it is automatically attached to VPC A. As a result, any applications in that environment are deployed to VPC A when the VPC is updated or restarted.
