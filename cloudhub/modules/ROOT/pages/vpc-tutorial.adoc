...
[NOTE]
If you create a new VPC or update an existing one and associate an environment that is used in another VPC in the same region, any applications deployed in that other VPC are automatically deployed in the new or updated VPC as soon as it is restarted or updated.
