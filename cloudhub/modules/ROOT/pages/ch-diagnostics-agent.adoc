...
What you get depends on your Mule runtime version:

* **Older Mule versions** (before DIAF support): Diagnostics are a thread dump—a snapshot of all threads and their current state for each worker.
* **Newer Mule versions** (with DIAF support): Diagnostics are a DIAF (Diagnostic Information Analysis File). The DIAF includes the thread dump plus the current runtime state in plain text and system details such as OS, Java version, and memory consumption.
