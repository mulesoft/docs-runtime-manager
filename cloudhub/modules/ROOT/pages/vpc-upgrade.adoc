...
The upgrade tool doesn't migrate or delete VPC peering connections. If a VPC with peering completes the upgrade, those connections remain on the CloudHub VPC. CloudHub apps can keep using them. Apps in the new CloudHub 2.0 private space can't use those peering connections.
// PLACEHOLDER (W-23544596): After CloudHub engineering confirms, add the customer-facing steps for:
// - Migrating from VPC peering to a transit gateway or VPN before you upgrade
// - What to do with leftover peering after the upgrade (leave it, delete it, who deletes it, and how)
...
// PLACEHOLDER (W-23544596): After CloudHub engineering confirms, add the customer-facing steps for migrating off VPC peering (and Direct Connect, if the same process applies) to a transit gateway or VPN.
