...
The VPC upgrade tool migrates the DLB certificates to the new private space on a best-effort basis only. If needed, you can create the DLB certificates on the new private space under the *Domains & TLS* tab. 
...
+
You can send only HTTP and HTTPS traffic to applications in the migrated private space. Upgraded private spaces don't support TCP traffic.
...
As soon as the upgrade process starts, the upgraded VPC gets locked for any further updates. 

You can cancel the upgrade process to revert to your VPC at any time by clicking *Cancel Upgrade*. In that case, the system deletes the newly created private space, and your VPC becomes unlocked.
...
Runtime Manager Tool:: Alternatively, migrate your applications using the application migration tool. See xref:app-migration.adoc[].
...
During the migration, you can continue to deploy and manage applications in your VPC to maintain business continuity. After the upgrade, your CloudHub dedicated VPC becomes locked for any changes to the infrastructure, including environment or business group associations, network connections, and internal DNS, to prevent configuration drift. Make any required changes in the newly created private space instead. The upgrade process applies any CloudHub 2.0 configuration changes to the CloudHub VPC. 
...
The firewall rules configured in your VPC get copied to the private space. After that, you can make additional changes to the firewall rules in the VPC and the private space. To avoid configuration drift, apply any firewall rule changes in both places throughout the migration.
