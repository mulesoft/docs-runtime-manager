...
* An Anypoint VPN with at least one pair of tunnels configured with BGP routing and DPD (Dead Peer Detection) to reset the tunnels when triggered incurs minimal downtime. The switchover takes a few seconds.
...
If you use Azure VPN, ensure that PFS is enabled and adjust the lifetimes for Phase 2. For more information, see https://help.salesforce.com/s/articleView?id=001116834&type=1[Anypoint VPN to Azure keeps going down every hour].

=== Anypoint VPN with BGP Routing Enters into Asymmetric Mode after an Update

Anypoint VPN enters into asymmetric mode after an on-demand maintenance if the AS paths for the two tunnels of the same VPN are different.

=== Seeing IPsec DPD Failures in VPN Logs

Ensure that your DPD settings are correctly configured. 
