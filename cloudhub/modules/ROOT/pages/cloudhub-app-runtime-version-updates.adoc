...
With blue-green deployments, when an automated update fails to be deployed to a new worker, the application will continue running the non-latest patch version on the currently running worker.

Failures could be due to (but not limited to): 

* After an application deployment, one of the application properties was removed from the ARM UI

* Incompatibility of a custom module with the latest patch version release


If an application fails to be automatically updated after 2 monthly patching cycles, the application will be stopped and force updated to the latest patch version. Applications that fail to start will require customers action to address patches. 
