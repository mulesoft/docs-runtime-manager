...
See https://help.salesforce.com/s/articleView?id=001116902&type=1[How to Perform a Bulk CloudHub Application Update to the Latest Runtime Releases].
...
After a new update occurs, you can check your xref:runtime-manager::managing-deployed-applications.adoc#application_status_states[application status state] in the *Status* column in Runtime Manager's *Applications* tab.
...
To be notified by e-mail when an error occurs after an automated update, you can xref:runtime-manager::alerts-on-runtime-manager.adoc#create-an-alert[create an alert]. 
...
[IMPORTANT]
Using a runtime version that is more than three months old can introduce security vulnerabilities. If your application fails to be automatically updated after two monthly patching cycles, the application is stopped and force-updated to the latest patch version. Applications that fail to start require your action to address patches. 
