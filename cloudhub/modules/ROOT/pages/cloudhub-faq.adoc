...
For more information, see https://blogs.mulesoft.com/api-integration/cloud/introducing-integration-paas-ipaas/[Introducing integration PaaS (iPaaS)].
...
An https://anypoint.mulesoft.com/home/[Anypoint Platform account] gives you access to both the *Runtime Manager* – for deploying and managing applications both on *CloudHub* and on other servers –  and to the Anypoint Platform for APIs – for creating and managing APIs through their entire lifecycle.
...
Anypoint Studio can be downloaded https://www.mulesoft.com/lp/dl/anypoint-mule-studio[here]. From Studio, you can deploy directly to CloudHub or run your applications locally on an embedded Mule runtime engine (Mule).
...
You can browse and search the online https://trailhead.salesforce.com/trailblazer-community/neighborhoods/mulesoft[MuleSoft Trailblazer Community] to find answers. Or post a question and start a new thread.
...
* *Security patching*
+
If any security vulnerabilities are identified, the relevant applications are patched and restarted. See the https://www.salesforce.com/en-us/wp-content/uploads/sites/4/documents/legal/Agreements/versioning-back-support-policy.pdf[Critical Security Vulnerabilities] policy.
* *Application crash*
+
...
MuleSoft is limiting the free accounts to one worker, which allows you to deploy one application. You can upgrade to a https://www.mulesoft.com/platform/saas/cloudhub-ipaas-cloud-based-integration[paid subscription] to deploy more applications, or you can scale your single application across more than one worker.
...
You can assign a static IP to your application by using the *Static IPs* application settings tab. For details, see xref:deploying-to-cloudhub.adoc#static-ips-tab-settings[Static IPs Tab Settings]. By default, you are allocated a number of static IPs equal to twice the number of production vCores in your subscription. To increase the number of static IPs allocated to your organization, reach out directly to your Account Team. Your Account Team can start the request process with you.
...
Because CloudHub deploys on Amazon EC2, IP addresses are chosen from the Amazon EC2 IP pool. For a list of these ranges, see https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-instance-addressing.html[Amazon EC2 Public IP Ranges].
