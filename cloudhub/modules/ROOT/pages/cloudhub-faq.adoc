...
You can assign a static IP to your application by using the *Static IPs* application settings tab. For details, see xref:deploying-to-cloudhub.adoc#static-ips-tab-settings[Static IPs Tab Settings]. By default, you are allocated a number of static IPs equal to twice the number of production vCores in your subscription. To increase the number of Static IP's allocated to your organization, please reach out directly to your Account Team. Your Account Team can start the request process with you.
