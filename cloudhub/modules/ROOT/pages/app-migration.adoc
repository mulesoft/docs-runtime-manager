...
After your Anypoint VPC upgrade is complete and all associated CloudHub applications are migrated and traffic is fully redirected to your CloudHub 2.0 application, an important step for configurations that involve Dedicated Load Balancers (DLBs) is to complete a DLB failover.

The DLB failover helps you retain the same domains as you transition from CloudHub to CloudHub 2.0. This ensures that client apps connecting to CloudHub remain compatible.
...
[NOTE]
You can roll back to CloudHub 1.0 if there are failures during this stage.


== Mark Upgrade as Complete

After you complete the individual application migrations, stop and delete your CloudHub 1.0 applications, and stop and delete any associated CloudHub 1.0 Dedicated Load Balancers via DLB failover, the final step for the VPC upgrade is to Mark Upgrade Complete.

To mark the upgrade process as complete, navigate to the Upgrade tab and click Mark Upgrade Complete.

[WARNING] 
This is an irreversible action. Clicking Mark Upgrade Complete deletes the original VPC, stopped Dedicated Load Balancers (DLBs), and all CloudHub 1.0 applications marked as upgraded. All traffic then flows through the CloudHub 2.0 private space.
