...
After the VPC upgrade is complete and the associated CloudHub applications are migrated to CloudHub 2.0, perform a DLB failover to retain your existing domains as you transition from CloudHub to CloudHub 2.0. This makes sure that client applications remain compatible.
...
. After the traffic failover completes, stop and delete the CloudHub 1.0 DLB linked to the VPC.
