...
. After the traffic failover completes, stop and delete the CloudHub 1.0 DLB that was linked to the VPC.

[NOTE]
If there are failures during this stage, restart the DLB to point `anypointdns.net` to CloudHub 1.0, provided the DLB is in a 'stopped' state. After you delete the DLB, you can't roll back to CloudHub 1.0.
