...
After your Anypoint VPC upgrade is complete and all associated CloudHub applications are migrated and traffic is fully redirected to your CloudHub 2.0 application, an important step for configurations that involve DLBs is to complete a DLB failover.

The DLB failover helps you retain the same domains as you transition from CloudHub to CloudHub 2.0. This makes sure that client applications connecting to CloudHub remain compatible.

To complete a DLB failover:

* For each linked DLB, failover the DLB domain `anypointdns.net` to CloudHub 2.0. This is important to remove an additional DLB hop and direct traffic to your new CloudHub 2.0 private space URL.
