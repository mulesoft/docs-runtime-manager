...
[NOTE]
====
If your CloudHub 1.0 application has the default endpoint configured (which results in traffic switching at the SLB) and you need to switch traffic from the DLB instead, remove the CloudHub 1.0 default endpoint, apply changes, and then change the traffic switch percentage. This allows traffic switching to take effect at the DLB.
====
