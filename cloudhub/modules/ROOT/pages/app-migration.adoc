...
* Update your CloudHub HTTP applications to become HTTPS. CloudHub 2.0 private spaces are configured by default to redirect traffic to HTTPS, so you can leave this option on the private space settings.
* Enable Last-Mile Security for the CloudHub 2.0 application.
* Enable Upstream TLS 1.2 or higher in your DLB settings if switching traffic from the DLB.
* Make sure that vanity domains on CloudHub 2.0 match those on the CloudHub application to prevent dropped traffic, or avoid setting a traffic switch percentage.
...
+
This option applies to all client-to-private-space traffic. If enabled, all applications within that private space accept HTTP traffic.
