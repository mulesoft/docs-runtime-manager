...
CloudHub 2.0 private spaces are configured by default to redirect traffic to HTTPS. However, this configuration does not redirect HTTP traffic sent by the legacy CloudHub application. To prevent HTTP requests from failing during traffic switching, enable the *Accept HTTP* option within the *Advanced* tab of the private space settings. This option allows CloudHub 2.0 to accept HTTP traffic. 
