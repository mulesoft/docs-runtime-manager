...
After migrating your applications to CloudHub 2.0, to manage a migrated app via the Mule Maven Plug-in (MMP), copy the same public URL as the one you configured on Runtime Manager during the migration. If the endpoints in your `pom.xml` file don't match the ones configured via the UI during the migration, the Ingress configuration gets lost.
