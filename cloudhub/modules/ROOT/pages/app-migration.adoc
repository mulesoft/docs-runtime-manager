...
• When properties are initially copied during CloudHub 2.0 application configuration, they do not persist unless you edit at least one property. Editing a single property causes the edited property and all other copied properties to persist.
...
When sending traffic via DLB in CloudHub applications, you can target either HTTP or HTTPS. This impacts on whether the DLB uses port 8091 or 8092 to forward requests to your application:

* If you target HTTP, the DLB receives traffic on HTTP and forwards it to port 8091. The app then receives on port 8091.
* If you target HTTPS, the DLB receives traffic on HTTPS and forwards it to port 8092. The app then receives on port 8092.
...
* For traffic switching involving a Dedicated Load Balancer (DLB), which typically operates on ports 8091 and 8092, add a firewall rule to allow HTTP from the local private network.
* For traffic switching involving a Shared Load Balancer (SLB), which typically operates on ports 8081 and 8082, add a firewall rule to allow HTTP from any source.
