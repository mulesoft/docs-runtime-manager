...
[WARNING]
If you use a different name for the cloudHub 2.0 application, make sure you input the CloudHub application name as part of the URL in the Ingress tab settings. Failing to configure the the correct URLs can sometimes result in traffic not reaching the CloudHub 2.0 application.
+
By default, this action enables DLB traffic switching. To enable SLB traffic switching, add the CloudHub application endpoint in the format: `<app_name>.<region>.cloudhub.io`.
