...
After your VPC upgrade is complete, you can start migrating all associated applications. You can do so by creating an equivalent CloudHub 2.0 application for every CloudHub application, and linking them to have weighted DNS records created for both the CloudHub and the CloudHub 2.0 URLs. 
...
== Upgrade your CH application to CH2 via Anypoint Platform

. From *Anypoint Platform*, select *Runtime Manager* > *Applications*.
. Select the CloudHub application you want to upgrade and navigate to *Settings*. 
. If your application is eligible for the upgrade, the upgrade tooling is available on the *Settings* page: click *Upgrade to CloudHub 2.0*, then click *Start Upgrade*.
+
