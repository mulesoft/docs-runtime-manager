...
If there's a hard change on traffic from one application to the other, policy counters behave as if they were reset. In a balanced round-robin traffic distribution, the effective number of requets can sometimes double as each application maintains an independent counting mechanism.
