...
When sending traffic via SLB in CloudHub applications, you can target either HTTP or HTTPS. This impacts on wether the SLB  uses port 8081 or 8082 to forward requests to your application: 

* If you target HTTP, the  SLB receives on HTTP and forwards to port 8081. The app then receives on port 8081.
* If you target HTTPS, the SLB receives on HTTPS and forwards to port 8082. The app then receives on port 8082.

CloudHub 2.0 private spaces are configured by default to redirect traffic to HTTPS. However, this configuration does not redirect HTTP traffic sent by the legacy CloudHub application. 

If you use a mix of HTTP and HTTPS application in CloudHub SLB and want to migrate this configuration, use either of these workarounds:

* Enable the *Accept HTTP* option within the *Advanced* tab of the private space settings for your CloudHub 2.0 application. This option allows CloudHub 2.0 to accept HTTP traffic. 
+
...
+
[NOTE]
This workaround can take more work and time depending how many HTTP CloudHub applications you have.
