...
You can view and control any Virtual Machine (VM) or Staged Event-Driven Architecture (https://en.wikipedia.org/wiki/Staged_event-driven_architecture[SEDA]) queues within the flows of your deployed applications from the *Queues* tab in Runtime Manager.
...
When you enable persistent queues, CloudHub replaces the default Mule runtime queue manager with a persistent one. This manager persists all queues, regardless of in-app configuration. Affected queue types:

* VM connector queues, whether configured as persistent or not in the application
* Internal Batch component queues
* Internal SEDA queues
