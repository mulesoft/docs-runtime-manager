...
When you enable persistent queues, CloudHub replaces Mule runtime's default queue manager with a persistent queue manager that treats all queues as persistent, regardless of the in-app configuration. The following queue types are affected:
