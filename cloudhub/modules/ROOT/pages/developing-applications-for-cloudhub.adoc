...
CloudHub worker local storage is ephemeral. Unlike local development environments, CloudHub clears local data when an app restarts.

This limitation affects:

* *Persistent object stores*: Configuring persistent="true" writes data to local disk. This works in Anypoint Studio and standalone Mule runtimes, but data doesn't persist across CloudHub restarts. Use xref:object-store::index.adoc[Object Store v2] for persistent storage.
* *Local file operations*: The File connector and other file-based operations write to the worker's local disk. CloudHub deletes these files when the app restarts.
* *Temporary files and caches*: App-generated files, custom logs, and local cache data don't persist.

To ensure data persistence in CloudHub:
...
For more information about designing resilient applications, see xref:cloudhub-hadr.adoc[CloudHub High Availability and Disaster Recovery].
