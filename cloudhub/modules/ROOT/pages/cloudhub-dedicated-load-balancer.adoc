...
+
If the target CloudHub application has multiple workers and they don't respond within the defined timeout, the DLB makes requests to the next worker sequentially. The xref:lb-create-arm#create-a-load-balancer.adoc[default timeout value] is 300 seconds.
+
