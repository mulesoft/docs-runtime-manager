...
+
If the target CloudHub application has multiple workers and they don't respond within the defined timeout, the DLB makes requests to the next worker sequentially. The xref:lb-create-arm.adoc#create-a-load-balancer[default timeout value] is 300 seconds.
+
...
* To avoid connection issues when the load balancer is restarting, adhere to the DNS TTL (30 seconds), including connection keep alive. Connection keep alive time is set to 60 seconds by default. The connections are closed after the new workers are online and the TTL expires.
...
* Proxy buffering is enabled by default in Anypoint Dedicated Load Balancers. This feature stores the response from a server in internal buffers as it comes in, and delays sending data to the client until the entire response is buffered. Proxy buffering helps optimizing performance with slow clients, as it allows the proxied server to return its response quickly and serve other requests. However, it is not ideal for small payload streaming, as the buffer bucket can take longer to fill in. To avoid performance problems in small payload streaming, disable proxy buffering by passing `no` in the `X-Accel-Buffering` response header field.
* CloudHub load balancers use Nginx HTTP server capabilities. Nginx reverse proxies ignore HTTP header fields with invalid names. Valid HTTP header names are composed of English letters, numbers, hyphens, and underscores. Only valid headers are forwarded to the Mule application. Use only valid header names to avoid any issues.
