...
+
For all alerts, alert notification email sending rates are set by the Mail Transfer Agent (MTA). An MTA is a service that transfers emails between a sender's and a recipient's email server.
The MTA stores an agreed rate between the email servers and sends emails below the agreed rate.
