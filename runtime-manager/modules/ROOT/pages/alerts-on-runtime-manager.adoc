...
+
For all alerts, alert notification email sending rates are set by the Mail Transfer Agent (MTA). An MTA is a service that transfers emails between a sender's and a recipient's email servers.
The MTA stores an agreed rate between the email servers and send emails below agreed rate.
