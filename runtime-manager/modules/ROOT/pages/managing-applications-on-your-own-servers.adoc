...
Sign in to the https://anypoint.mulesoft.com[Anypoint Platform] and view the *Applications* tab of the Runtime Manager. Select one of your CloudHub running applications to open a panel. On that panel, click the *Manage Application* button to open up the application's settings.

image::managing-applications-on-your-own-servers-1fd39.png[]

In this menu you can view all of the same data that's available on the panel, with the addition of an *Alerts History* tab, that displays a record of the triggered alerts on this app. See xref:alerts-on-runtime-manager.adoc[Alerts on Runtime Manager] for more on this.
...
* If you manually delete an application folder or anchor file while the Mule runtime is running, the affected applications are un-deployed from that local server. The applications on other nodes in a server group or cluster are not affected.
[NOTE]
...
* When the Mule runtime restarts, it triggers a re-sync process. During the re-sync, Runtime Manager detects the missing applications and automatically redeploys them. This applies to single servers, server groups, and clusters.
[NOTE]
The re-sync only occurs when the local Mule runtime restarts.
...
However, you can download the archive using an Runtime Manager REST Services API call or the Anypoint xref:anypoint-cli::standalone-apps.adoc#runtime-mgr-standalone-application-artifact[command-line interface].
