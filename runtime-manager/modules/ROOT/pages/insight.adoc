...
This page documents a legacy troubleshooting tool for Mule apps deployed to CloudHub and on-premises servers. For the new troubleshooting approach with traces, refer to these pages: xref:runtime-manager::runtime-manager/insight.adoc#enable-trace-data[Enable Trace Data Collection for CloudHub Apps] and xref:monitoring::am-installing.adoc#tracing-setup[Enable Trace Data Collection for Hybrid Runtime Servers].
--
