...
This page documents a legacy troubleshooting tool for Mule apps deployed to CloudHub and on-premises servers. For the new troubleshooting approach with traces, refer to these pages: xref:runtime-manager::insight.adoc#enable-trace-data[Enable Trace Data Collection for CloudHub Apps] and xref:monitoring::am-installing.adoc#tracing-setup[Enable Trace Data Collection for Hybrid Runtime Servers].
--
...
Replay is not supported for free or trial user accounts.
...
. Click *Applications* in the menu on the left, and click the application in the *Name* column to open the management panel on the right side of the page.
...
+
The *Metadata and Replay* option is unavailable for free or trial user accounts.
+
...
Replay is not available for free or trial user accounts.
...
When you enable trace data collection for an app, Anypoint Platform collects and exposes tracing information from Mule runtime engine in the OpenTelemetry standard, enabling you to export it using the Anypoint Monitoring Distributed Tracing Experience. 

You can enable trace data collection for your CloudHub applications running on Mule version 4.6 or later.

To enable trace data collection for an app:

. In Runtime Manager, click the *Runtime* tab.
...
* https://blogs.mulesoft.com/dev-guides/how-to-tutorials/announcing-cloudhub-insight-real-time-visibility-message-replay-and-root-cause-analysis/[CloudHub Insight on the MuleSoft Blog]
