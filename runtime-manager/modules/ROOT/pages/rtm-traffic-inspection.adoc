...
** The control plane presents a Mulesoft Public server certificate to the Inspection Proxy. The proxy must be provisioned with the right Certificate Authority to validate the server certificate presented by the Mulesoft Control Plane
...
Make sure that the version of the Mule Runtime Agent is `2.5.6`. If the version is 2.5.6 or later, skip to the next section. If you have an earlier version, update the runtime agent before registering.
+
To upgrade the Runtime Agent:
+
...
. Update the file `wrapper.conf` with the IP and port of the traffic inspection proxy by following instructions in xref:rtm-agent-proxy-config.adoc#set-up-proxy-server-configuration-in-the-wrapper-conf-file[].
...
+
[NOTE]
Note the space between the proxy's domain name and port number.
+
...
* For agent version 2.5.6, you cannot renew mule certificates from Runtime Manager. If you need to renew your certificates, follow the instructions in xref:servers-cert-renewal.adoc#renew-a-certificate-via-the-command-line[]. 
...
** Using a runtime version earlier than the one specified during the installation might result in some functionality not working as expected.
