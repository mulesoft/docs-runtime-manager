...
* *JDK versions*
+
The version of JDK that CloudHub implements for all applications built with Mule runtime 3.9.x, 4.1.x, 4.2.x and later is JDK 8.
+
A Mule application built and compiled against a JDK version higher than the JDK version the runtime is deployed to may cause a compatibility issue.
+
See xref:release-notes::cloudhub/cloudhub-runtimes-release-notes.adoc[CloudHub Runtime and Operating System Release Notes] to determine the minimum recommended JDK version for building and compiling applications.
* *Load balancing*
+
