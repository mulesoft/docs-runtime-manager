...
[NOTE]
As part of our continuous infrastructure improvement, Mulesoft is introducing a new set of static ip addresses in Runtime Manager that will augment the existing ones. To avoid service disruption, allow the following IP addresses or domain (depending on your network policy) through your organization's firewall before the February 2024 release as listed below.
