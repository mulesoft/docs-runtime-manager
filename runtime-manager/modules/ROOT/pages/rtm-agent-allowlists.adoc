...
These endpoints use mutual TLS (mTLS) authentication:
...
[IMPORTANT]
The `*.ingest.mulesoft.com:8443` endpoint uses Mutual TLS (mTLS) for direct agent-to-platform certificate authentication. Configure your firewall or network proxy to allow SSL passthrough — do not enable deep packet inspection (DPI) or SSL decryption for this traffic. Ensure proxy rules also permit outbound TCP on port 8443 in addition to standard port 443.

These endpoints use WebSockets:
...
[NOTE]
Starting with Runtime Manager Agent 2.7.0 (Anypoint Monitoring Agent 2.7.1+), Mule runtimes stream telemetry metrics using an OpenTelemetry-based exporter. This requires outbound access to `*.ingest.mulesoft.com` on port 8443.
...
|*EU*|s3.eu-central-1.amazonaws.com |  443
...
Allowlist the following URLs, the valid Agents are only 2.2.0+:
